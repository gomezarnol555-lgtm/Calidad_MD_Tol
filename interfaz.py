...
from base_datos import consultar, crear, editar, eliminar, guardar_varios, transaccion, operacion_datos, ErrorIntegridad, reiniciar_consecutivo, inicializar_base
...
def cambiar_password_obligatoria():
    ...
    if guardar:
        ...
        if errores:
            ...
            crear(operacion_datos('INTERFAZ_0028'), (hash_password(nueva), auth['usuario']))
...
def styles(compact=False):
    st.markdown(operacion_datos('INTERFAZ_0029'), unsafe_allow_html=True)
...
def panel_indicadores_spac_inicio(indicador='SPAC'):
    ...
    registros = consultar(operacion_datos('INTERFAZ_0030'))
...
def _panel_registros_inicio(tipo):
    ...
    d = consultar(operacion_datos('INTERFAZ_0031', tabla))
    ...
    if defecto == '_codigo_defecto_panel':
        cat = consultar(operacion_datos('INTERFAZ_0032'))
...
def editor_metas_cumplimiento(tipo_entidad, entidades, teoricos, periodo_tipo, periodo_clave, key):
    ...
    guardados = consultar(operacion_datos('INTERFAZ_0033'), (tipo_entidad, periodo_tipo, periodo_clave))
        ...
        if st.button('Guardar datos cargados y calcular cumplimiento', key=key + '_guardar', type='primary'):
            ...
            try:
                for _, r in editado.iterrows():
                    cur.execute(operacion_datos('INTERFAZ_0034'), (tipo_entidad, str(r['Entidad']), periodo_tipo, periodo_clave, float(r['Datos cargados'] or 0), st.session_state.auth['usuario'], now_iso()))
                c.commit()
            except Exception:
                c.rollback()
                raise
        ...
        if errores:
            ...
            try:
                for entidad, fecha_columna, valor in cambios:
                    if valor is None:
                        cur.execute(operacion_datos('INTERFAZ_0035'), (tipo_entidad, entidad, fecha_columna))
                    else:
                        cur.execute(operacion_datos('INTERFAZ_0036'), (tipo_entidad, entidad, fecha_columna, float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso()))
                c.commit()
            except Exception:
                c.rollback()
                raise
...
def matriz_entregas():
    ...
    df = consultar(operacion_datos('INTERFAZ_0037'))
    if df.empty:
        ...
        if not coincidencia.empty:
            ...
            if is_dev() and st.session_state.get('mx_confirm') == rid:
                ...
                if x.button('Confirmar eliminación', key=f'mx_ok_{rid}'):
                    ...
                    try:
                        if eid:
                            cur.execute(operacion_datos('INTERFAZ_0038'), (eid,))
                            cur.execute(operacion_datos('INTERFAZ_0039'), (eid,))
                            cur.execute(operacion_datos('INTERFAZ_0040'), (eid,))
                        cur.execute(operacion_datos('INTERFAZ_0041'), (rid,))
                        cdb.commit()
                    except Exception:
                        cdb.rollback()
                        raise
...
def page_registro():
    ...
    def form_hallazgo(tabla, titulo, audit_action):
        ...
        prod = consultar(operacion_datos('INTERFAZ_0042'))
        defs = consultar(operacion_datos('INTERFAZ_0043'))
    ...
    def form_devoluciones():
        ...
        if st.button('← Cambiar tipo de registro', key='volver_devoluciones'):
            volver_selector()
        st.markdown('<div class="registro-form-shell">', unsafe_allow_html=True)
        prod = consultar(operacion_datos('INTERFAZ_0045'))
        defs = consultar(operacion_datos('INTERFAZ_0046'))
        opt_prod = [''] + [f'{r.item} | {r.descripcion}' for r in prod.itertuples()]
        opt_defs = [''] + [f'{r.codigo} | {r.defecto}' for r in defs.itertuples()]
        a, b, c = st.columns(3)
        ...
        if st.button('Guardar registro', key=f'guardar_dev_{nonce}', type='primary'):
            ...
            if faltan:
                ...
                rid = crear(operacion_datos('INTERFAZ_0047'), (fecha.isoformat(), codigo, defecto, cedis.strip(), pais_estado.strip(), item, producto, cliente, '', familia, lote.strip(), caducidad.strip(), descripcion.strip(), comprobado, float(cantidad), unidad, sector.strip(), disposicion, tsp.strip(), status, nave, st.session_state.auth['usuario'], now_iso()))
    ...
    def form_reclamos():
        ...
        if st.button('← Cambiar tipo de registro', key='volver_reclamos'):
            volver_selector()
        st.markdown('<div class="registro-form-shell">', unsafe_allow_html=True)
        prod = consultar(operacion_datos('INTERFAZ_0048'))
        defs = consultar(operacion_datos('INTERFAZ_0049'))
        opt_prod = [''] + [f'{r.item} | {r.descripcion}' for r in prod.itertuples()]
        opt_defs = [''] + [f'{r.codigo} | {r.defecto}' for r in defs.itertuples()]
        a, b, c = st.columns(3)
        ...
        if st.button('Guardar registro', key=f'guardar_reclamos_{nonce}', type='primary'):
            ...
            duplicado = not consultar(operacion_datos('INTERFAZ_0050'), (numero_caso,)).empty if numero_caso.strip() else False
            if duplicado:
                ...
                try:
                    rid = crear(operacion_datos('INTERFAZ_0051'), (fecha.isoformat(), codigo, defecto, fuente.strip(), mercado.strip(), pais_estado.strip(), numero_caso.strip(), item, producto, cliente, familia, descripcion_reclamo.strip(), causa_raiz.strip(), acciones.strip(), comprobado, float(cantidad), unidad, sector.strip(), estado, fecha_cierre.isoformat() if fecha_cierre else None, tsp.strip(), caducidad.strip(), lote.strip(), nave, pmd, tipo_defecto, clasificacion, red_social, st.session_state.auth['usuario'], now_iso()))
    ...
    def form_pnc():
        ...
        if st.button('← Cambiar tipo de registro', key='volver_pnc'):
            volver_selector()
        st.markdown('<div class="registro-form-shell">', unsafe_allow_html=True)
        prod = consultar(operacion_datos('INTERFAZ_0052'))
        defs = consultar(operacion_datos('INTERFAZ_0053'))
        opt_prod = [''] + [f'{r.item} | {r.descripcion}' for r in prod.itertuples()]
        opt_defs = [''] + [f'{r.codigo} | {r.defecto}' for r in defs.itertuples()]
        with st.container():
            a, b, c = st.columns(3)
...
def page_consulta():
        ...
        if st.session_state.get(f'confirm_del_{key}') == selected:
            ...
            with c1:
                if st.button('Confirmar eliminación', key=f'ok_del_{key}_{selected}'):
                    crear(operacion_datos('INTERFAZ_0055', table_name), (selected,))
    ...
    def edit_record(table_name, key, selected):
        row_df = consultar(operacion_datos('INTERFAZ_0056', table_name), (selected,))
        if row_df.empty:
            st.warning('El registro seleccionado ya no está disponible.')
            return
        row = row_df.iloc[0].to_dict()
        prod = consultar(operacion_datos('INTERFAZ_0057'))
        defs = consultar(operacion_datos('INTERFAZ_0058'))
        opt_prod = [''] + [f'{r.item} | {r.descripcion}' for r in prod.itertuples()]
        opt_defs = [''] + [f'{r.codigo} | {r.defecto}' for r in defs.itertuples()]
        ...
        with st.expander('✏️ Editar o completar registro', expanded=True):
            ...
            if guardar:
                ...
                if missing:
                    ...
                    try:
                        columnas_tabla = {r[1] for r in c_esquema.execute(operacion_datos('INTERFAZ_0059', table_name)).fetchall()}
                    finally:
                        c_esquema.close()
                    ...
                    crear(operacion_datos('INTERFAZ_0060', table_name, assignments), tuple((updates[c] for c in update_cols)) + (selected,))
    ...
    with t1:
        df = consultar(operacion_datos('INTERFAZ_0061'))
    ...
    with t2:
        df = consultar(operacion_datos('INTERFAZ_0062'))
    ...
    with t3:
        df = consultar(operacion_datos('INTERFAZ_0063'))
        campos_actuales = ['id', 'dia', 'mes', 'anio', 'semana', 'nave', 'item', 'producto', 'familia', 'lote', 'linea_sector', 'codigo_defecto', 'turno', 'supervisor_responsable', 'analista_detecta', 'descripcion_hallazgo', 'acciones_inmediatas', 'equipo_hallazgo', 'tipo', 'particulas_halladas', 'investigacion_origen', 'acciones_evitar_incidencia', 'creado_por', 'creado_en', 'actualizado_por', 'actualizado_en']
        disponibles = [c for c in campos_actuales if c in df.columns]
        df_tabla = df[disponibles].copy()
        if all((c in df_tabla.columns for c in ['dia', 'mes', 'anio'])):
    ...
    with t4:
        df = consultar(operacion_datos('INTERFAZ_0064'))
        ...
        if selected:
            ...
            row_df = consultar(operacion_datos('INTERFAZ_0065'), (selected,))
            if row_df.empty:
                st.warning('El registro seleccionado ya no está disponible.')
            else:
                row = row_df.iloc[0].to_dict()
                prod = consultar(operacion_datos('INTERFAZ_0066'))
                defs = consultar(operacion_datos('INTERFAZ_0067'))
                opt_prod = [''] + [f'{r.item} | {r.descripcion}' for r in prod.itertuples()]
                opt_defs = [''] + [f'{r.codigo} | {r.defecto}' for r in defs.itertuples()]
                ...
                with st.expander('✏️ Editar registro de Reclamo', expanded=True):
                    ...
                    if guardar:
                        ...
                        faltan = [k for k, v in req.items() if v is None or (isinstance(v, str) and (not v.strip())) or (k == 'Cantidad afectada' and float(v) <= 0)]
                        duplicado = not consultar(operacion_datos('INTERFAZ_0068'), (numero_caso, selected)).empty if numero_caso.strip() else False
                        if duplicado:
    ...
    with t5:
        df = consultar(operacion_datos('INTERFAZ_0070'))
        ...
        if selected:
            ...
            row_df = consultar(operacion_datos('INTERFAZ_0071'), (selected,))
            if row_df.empty:
                st.warning('El registro seleccionado ya no está disponible.')
            else:
                r = row_df.iloc[0].to_dict()
                prod = consultar(operacion_datos('INTERFAZ_0072'))
                defs = consultar(operacion_datos('INTERFAZ_0073'))
                ...
                with st.expander('✏️ Editar registro de Devolución', expanded=True):
                    ...
                    if guardar:
                        ...
                        if faltan:
                            ...
                            crear(operacion_datos('INTERFAZ_0074'), (fecha.isoformat(), codigo, defecto, cedis.strip(), pais.strip(), item, producto, cliente, familia, lote.strip(), cad.strip(), desc.strip(), comp, float(cant), unidad, sector.strip(), disp, tsp.strip(), status, nave, st.session_state.auth['usuario'], now_iso(), selected))
...
def page_muestras_retencion():
    ...
    productos = consultar(operacion_datos('INTERFAZ_0075'))
    ...
    if guardar:
        ...
        if faltan:
            ...
            rid = crear(operacion_datos('INTERFAZ_0076', tabla), (item, descripcion, lote.strip(), destino, float(muestras), float(corrugado), responsable, observaciones.strip(), st.session_state.auth['usuario'], now_iso()))
...
def consulta_muestras_retencion():
    ...
    for tab, (tabla, nombre) in zip(tabs, configuracion):
        with tab:
            ...
            original_df = consultar(operacion_datos('INTERFAZ_0079', tabla), (rid,))
            ...
            with st.expander(f'✏️ Editar registro N° {rid}', expanded=True):
                ...
                if actualizar:
                    ...
                    if faltan:
                        ...
                        crear(operacion_datos('INTERFAZ_0080', tabla), (item, descripcion, lote.strip(), destino, float(muestras), float(corrugado), responsable, observaciones.strip(), st.session_state.auth['usuario'], now_iso(), rid))
            ...
            if is_dev():
                ...
                if st.session_state.get(f'confirm_{tabla}') == rid:
                    ...
                    if d1.button('Confirmar eliminación', key=f'ok_{tabla}_{rid}'):
                        crear(operacion_datos('INTERFAZ_0081', tabla), (rid,))
...
def page_entrega_turno():
    ...
    if nave in ['Nave 2', 'Nave 3']:
        ...
        if st.button('Guardar entrega de turno', type='primary', key=f'et23_g_{nave}_{n}'):
            ...
            repetida = not consultar(operacion_datos('INTERFAZ_0082'), (fecha.isoformat(), analista)).empty
            faltan = []
            if not analista:
            ...
            if errores_seguimiento:
                ...
                eid = crear(operacion_datos('INTERFAZ_0083'), (nave, fecha.isoformat(), analista, turno, referencia, total_c, total_h, st.session_state.auth['usuario'], now_iso()))
                for grupo, linea, producto, horas, carga, obs, orden, nvcat in filas_clasificadas:
                    if producto.strip() or horas or carga or obs.strip():
                        crear(operacion_datos('INTERFAZ_0084'), (eid, grupo, linea, producto.strip(), horas, carga, obs.strip(), orden, nvcat))
                ...
                for j, (grupo, linea, tipo_analisis, resultado, obs, contador) in enumerate(resultados):
                    if resultado.strip() or obs.strip():
                        crear(operacion_datos('INTERFAZ_0085'), (eid, 'ANÁLISIS ' + grupo, linea, tipo_analisis, 0, contador, (resultado + ' | ' + obs).strip(' |'), base_orden + j))
                for bloque, df, meta in seguimientos:
                    for orden, row in df.iterrows():
                        ...
                        if any((v.strip() for v in datos.values())):
                            ...
                            crear(operacion_datos('INTERFAZ_0086'), (eid, bloque, *legado, json.dumps(datos, ensure_ascii=False), int(orden)))
    ...
    if st.button('Guardar entrega de turno', type='primary', key=f'et_g_{n}'):
        ...
        for bloque, df_seg, meta in seguimientos:
            for _, row_seg in df_seg.iterrows():
                ...
                if any(datos_seg.values()):
                    ...
                    if faltan_seg:
                        errores_seguimiento.append(f'{bloque}: ' + ', '.join(faltan_seg))
        repetida = not consultar(operacion_datos('INTERFAZ_0087'), (fecha.isoformat(), analista)).empty
        faltan = []
        if not analista:
            faltan.append('Analista')
        if not turno:
        ...
        if errores_seguimiento:
            ...
            eid = crear(operacion_datos('INTERFAZ_0088'), (nave, fecha.isoformat(), analista, turno, referencia, total_c, total_h, st.session_state.auth['usuario'], now_iso()))
            for grupo, linea, producto, horas, carga, obs, orden, nvcat in filas_clasificadas:
                if producto.strip() or horas or carga or obs.strip():
                    crear(operacion_datos('INTERFAZ_0089'), (eid, grupo, linea, producto.strip(), horas, carga, obs.strip(), orden, nvcat))
            for bloque, df, meta in seguimientos:
                for orden, row in df.iterrows():
                    datos = {str(col): '' if pd.isna(val) else str(val) for col, val in row.to_dict().items()}
                    if any((v.strip() for v in datos.values())):
                        ...
                        if obligatorios:
                            ...
                            continue
                        legado = [datos.get(c, '') for c in ['Registro #', 'Hoja física', 'Carga electrónica', 'Correo', 'Descripción del seguimiento']]
                        crear(operacion_datos('INTERFAZ_0090'), (eid, bloque, *legado, json.dumps(datos, ensure_ascii=False), int(orden)))
            guardar_matriz(eid, fecha.isoformat(), analista, total_c, totales_nave)
...
def page_catalogos():
    if not is_dev():
        ...
        productos = consultar(operacion_datos('INTERFAZ_0091'))
        ...
        with st.expander('Agregar producto', expanded=True):
            ...
            if agregar:
                ...
                existente = consultar(operacion_datos('INTERFAZ_0092'), (valores[0],)) if valores[0] else pd.DataFrame()
                if faltantes:
                    ...
                    crear(operacion_datos('INTERFAZ_0093'), (valores[1], valores[2], valores[3], int(existente.iloc[0].id)))
                    audit(st.session_state.auth['usuario'], 'REACTIVAR_PRODUCTO', f'ITEM {valores[0]}')
                    st.success('Producto agregado correctamente.')
                    st.rerun()
                else:
                    crear(operacion_datos('INTERFAZ_0094'), tuple(valores))
        ...
        with st.expander(f'Agregar {titulo_singular.lower()}', expanded=False):
            ...
            if agregar:
                ...
                if obligatorios:
                    ...
                    st.rerun()
        if rid:
            actual = consultar(operacion_datos('INTERFAZ_0095', tabla), (rid,))
            if not actual.empty:
                r = actual.iloc[0]
                ...
                if st.session_state.get(f'cat_{clave}_confirmar') == rid:
                    ...
                    if x.button('Confirmar eliminación', key=f'cat_{clave}_ok_{rid}'):
                        crear(operacion_datos('INTERFAZ_0096', tabla), (rid,))
    ...
    with tab1:
        ...
        catalogo_seleccionable('productos', operacion_datos('INTERFAZ_0097'), {'id': 'ID', 'item': 'ITEM', 'descripcion': 'Descripción', 'cliente': 'Cliente', 'familia': 'Familia'}, 'productos', 'Producto', [('item', 'ITEM *'), ('descripcion', 'Descripción *'), ('cliente', 'Cliente *'), ('familia', 'Familia *')], operacion_datos('INTERFAZ_0098'), operacion_datos('INTERFAZ_0099'), operacion_datos('INTERFAZ_0100'))
    with tab2:
        st.subheader('Defectos')
        st.caption('Selecciona una fila para editarla o eliminarla. Los cambios se aplican automáticamente en los formularios relacionados.')
        catalogo_seleccionable('defectos', operacion_datos('INTERFAZ_0101'), {'id': 'ID', 'codigo': 'Código', 'defecto': 'Defecto', 'tipo_defecto': 'Tipo de defecto', 'clasificacion': 'Clasificación'}, 'defectos', 'Defecto', [('codigo', 'Código *'), ('defecto', 'Defecto *'), ('tipo_defecto', 'Tipo de defecto *'), ('clasificacion', 'Clasificación *')], operacion_datos('INTERFAZ_0102'), operacion_datos('INTERFAZ_0103'), operacion_datos('INTERFAZ_0104'), {'tipo_defecto': catalog('tipo_defecto') or ['Funcional', 'Contaminación'], 'clasificacion': ['Inocuidad', 'Calidad', 'Salubridad', 'Legalidad']})
    with tab3:
        ...
        raw = consultar(operacion_datos('INTERFAZ_0105'))
        ...
        with st.expander('Agregar valor', expanded=False):
            ...
            if agregar:
                ...
                elif not consultar(operacion_datos('INTERFAZ_0106'), (etiquetas[lista], valor.strip())).empty:
                    st.error('El valor ya existe en la lista seleccionada.')
                else:
                    crear(operacion_datos('INTERFAZ_0107'), (etiquetas[lista], valor.strip()))
        ...
        if rid:
            ...
            with st.expander('Editar valor seleccionado', expanded=True):
                ...
                if guardar:
                    cat = etiquetas[lista]
                    val = valor.strip()
                    dup = consultar(operacion_datos('INTERFAZ_0108'), (cat, val, rid))
                    if not val:
                        st.error('Ingresa un valor válido.')
                    elif not dup.empty:
                        st.error('Ya existe otro valor igual en la lista seleccionada.')
                    else:
                        crear(operacion_datos('INTERFAZ_0109'), (cat, val, rid))
            ...
            if st.session_state.get('cat_datos_confirmar') == rid:
                ...
                if x.button('Confirmar eliminación', key=f'cat_datos_ok_{rid}'):
                    crear(operacion_datos('INTERFAZ_0110'), (rid,))
    ...
    with tab4:
        ...
        df_general = consultar(operacion_datos('INTERFAZ_0111'))
        if filtro_nave:
            ...
            with st.expander('Agregar relación general', expanded=False):
                ...
                if agregar_general:
                    ...
                    if not li or not se:
                        ...
                        duplicado = consultar(operacion_datos('INTERFAZ_0112'), (nueva_nave, normalizar_catalogo(li), normalizar_catalogo(se)))
                        if not duplicado.empty:
                            st.error('La relación ya existe en el catálogo general.')
                        else:
                            orden = int(consultar(operacion_datos('INTERFAZ_0113'), (nueva_nave,)).iloc[0]['n'])
                            crear(operacion_datos('INTERFAZ_0114'), (nueva_nave, li, se, normalizar_catalogo(li), normalizar_catalogo(se), orden))
                            ...
                            st.rerun()
            if id_general:
                actual = consultar(operacion_datos('INTERFAZ_0115'), (id_general,))
                if not actual.empty:
                    r = actual.iloc[0]
                    with st.expander('Editar relación general seleccionada', expanded=True):
                        ...
                        if guardar_general:
                            li = linea_e.strip()
                            se = sector_e.strip()
                            duplicado = consultar(operacion_datos('INTERFAZ_0116'), (nave_e, normalizar_catalogo(li), normalizar_catalogo(se), id_general))
                            if not li or not se:
                                st.error('Completa Línea y Sector.')
                            elif not duplicado.empty:
                                st.error('Ya existe otra relación activa con esos datos.')
                            else:
                                crear(operacion_datos('INTERFAZ_0117'), (nave_e, li, se, normalizar_catalogo(li), normalizar_catalogo(se), int(orden_e), id_general))
                    ...
                    if st.session_state.get('cat_general_confirmar') == id_general:
                        ...
                        if x.button('Sí, eliminar', key=f'cat_general_ok_{id_general}'):
                            crear(operacion_datos('INTERFAZ_0118'), (id_general,))
    ...
    with tab5:
        ...
        df = consultar(operacion_datos('INTERFAZ_0119'), (nave_fmt, tipo_fmt))
            ...
            with st.expander('Agregar elemento al formato', expanded=False):
                ...
                if agregar:
                    ...
                    if faltan:
                        ...
                        existente = consultar(operacion_datos('INTERFAZ_0120'), (nave_fmt, tipo_fmt, linea.strip(), sector.strip(), analisis.strip() if tipo_fmt == 'ANALISIS' else ''))
                        if existente.empty:
                            crear(operacion_datos('INTERFAZ_0121'), (nave_fmt, tipo_fmt, linea.strip(), sector.strip(), analisis.strip() if tipo_fmt == 'ANALISIS' else '', int(ol), int(os)))
                        else:
                            crear(operacion_datos('INTERFAZ_0122'), (int(ol), int(os), int(existente.iloc[0].id)))
                        ...
                        st.rerun()
            if rid:
                r = consultar(operacion_datos('INTERFAZ_0123'), (rid,))
                if not r.empty:
                    r = r.iloc[0]
                    with st.expander('Editar elemento seleccionado', expanded=True):
                        ...
                        if guardar:
                            ...
                            if not linea_editada or not sector_editado or (tipo_fmt == 'ANALISIS' and (not analisis_editado)):
                                ...
                                duplicado = consultar(operacion_datos('INTERFAZ_0124'), (nave_fmt, tipo_fmt, linea_editada, sector_editado, analisis_editado, rid))
                                if not duplicado.empty:
                                    st.error('No se puede guardar porque ya existe otro elemento activo con la misma Nave, Línea, Sector y Tipo de análisis.')
                                else:
                                    try:
                                        crear(operacion_datos('INTERFAZ_0125'), (linea_editada, sector_editado, analisis_editado, int(ol), int(os), rid))
                                    except ErrorIntegridad:
                    ...
                    if st.session_state.get('fmt_confirm') == rid:
                        ...
                        if x.button('Sí, eliminar', key=f'fmt_ok_{rid}'):
                            crear(operacion_datos('INTERFAZ_0126'), (rid,))
        ...
        seg = consultar(operacion_datos('INTERFAZ_0127'))
        ...
        with st.expander('Agregar seguimiento'):
            ...
            if alta:
                nombre = nom.strip()
                ex = consultar(operacion_datos('INTERFAZ_0128'), (nombre,)) if nombre else pd.DataFrame()
                if not nombre:
                    st.error('Ingresa el seguimiento.')
                elif not ex.empty:
                    crear(operacion_datos('INTERFAZ_0129'), (nombre, int(orden), int(ex.iloc[0].id)))
                    audit(st.session_state.auth['usuario'], 'REACTIVAR_SEGUIMIENTO_ENTREGA', nombre)
                    st.session_state.seg_nonce = ns + 1
                    st.rerun()
                else:
                    crear(operacion_datos('INTERFAZ_0130'), (nombre, int(orden)))
                    audit(st.session_state.auth['usuario'], 'AGREGAR_SEGUIMIENTO_ENTREGA', nombre)
                    st.session_state.seg_nonce = ns + 1
                    st.rerun()
        if sid:
            rr = consultar(operacion_datos('INTERFAZ_0131'), (sid,)).iloc[0]
            ...
            campos_seg = consultar(operacion_datos('INTERFAZ_0132'), (sid,))
            ...
            with st.expander('Agregar campo al seguimiento'):
                ...
                if ca:
                    ...
                    if not nombre_campo:
                        ...
                        crear(operacion_datos('INTERFAZ_0133'), (sid, nombre_campo, ct, opciones_campo, int(cob), int(cord)))
            ...
            if cid:
                ...
                with st.expander('Editar campo seleccionado', expanded=True):
                    ...
                    if ceg:
                        ...
                        if not cen.strip():
                            ...
                            crear(operacion_datos('INTERFAZ_0134'), (cen.strip(), cet, opciones_edit, int(ceob), int(ceord), cid))
                ...
                if st.button('Eliminar campo seleccionado', key=f'seg_campo_del_{cid}'):
                    crear(operacion_datos('INTERFAZ_0135'), (cid,))
            ...
            with st.expander('Editar seguimiento seleccionado', expanded=True):
                ...
                if guardar and ne.strip():
                    crear(operacion_datos('INTERFAZ_0136'), (ne.strip(), int(oe), sid))
            ...
            if st.session_state.get('seg_confirm') == sid:
                ...
                if a.button('Confirmar eliminación', key=f'seg_ok_{sid}'):
                    crear(operacion_datos('INTERFAZ_0137'), (sid,))
...
def page_usuarios():
    ...
    bloqueados = consultar(operacion_datos('INTERFAZ_0138'))
    with st.expander(f'🔓 Habilitación de cuentas bloqueadas ({len(bloqueados)})', expanded=not bloqueados.empty):
        if bloqueados.empty:
            ...
            if st.button('Reactivar cuenta y reiniciar intentos', type='primary', key='reactivar_usuario_bloqueado'):
                ...
                crear(operacion_datos('INTERFAZ_0139'), (bid,))
    ...
    with st.expander('Agregar nuevo usuario', expanded=False):
        ...
        if crear:
            ...
            if faltan:
                st.error('Completa los campos obligatorios: ' + ', '.join(faltan) + '.')
            elif not consultar(operacion_datos('INTERFAZ_0140'), (usuario,)).empty:
                st.error('No fue posible crear la cuenta porque el nombre de usuario ya está registrado.')
            else:
                crear(operacion_datos('INTERFAZ_0141'), (usuario, nombre, hash_password(password), nuevo_rol, now_iso()))
                audit(st.session_state.auth['usuario'], 'CREAR_USUARIO', f'Usuario {usuario} | Rol {nuevo_rol}')
                st.session_state.usuarios_nonce += 1
                st.success('Usuario creado correctamente.')
                st.rerun()
    st.subheader('Usuarios registrados')
    usuarios = consultar(operacion_datos('INTERFAZ_0142'))
    if usuarios.empty:
    ...
    actual_df = consultar(operacion_datos('INTERFAZ_0143'), (rid,))
    if actual_df.empty:
        st.session_state.usuarios_nonce += 1
        st.rerun()
    r = actual_df.iloc[0]
    ...
    with st.expander('Editar información y rol', expanded=True):
        ...
        if guardar:
            ...
            if not usuario or not nombre:
            ...
            elif not consultar(operacion_datos('INTERFAZ_0144'), (usuario, rid)).empty:
                ...
                if nueva:
                    crear(operacion_datos('INTERFAZ_0145'), (usuario, nombre, rol_editado, hash_password(nueva), rid))
                else:
                    crear(operacion_datos('INTERFAZ_0146'), (usuario, nombre, rol_editado, rid))
                audit(st.session_state.auth['usuario'], 'EDITAR_USUARIO', f'ID {rid} | Usuario {usuario} | Rol {rol_editado}')
                if rid == int(consultar(operacion_datos('INTERFAZ_0147'), (st.session_state.auth['usuario'],)).iloc[0].id):
    ...
    if st.session_state.get('usuario_confirmar_eliminacion') == rid:
        ...
        if x.button('Confirmar eliminación', key=f'confirmar_usuario_{rid}', use_container_width=True):
            if str(r['usuario']) == st.session_state.auth['usuario']:
                ...
                crear(operacion_datos('INTERFAZ_0150'), (rid,))
...
def page_auditoria():
    st.title('Auditoría')
    st.dataframe(consultar(operacion_datos('INTERFAZ_0151')), use_container_width=True, hide_index=True)
