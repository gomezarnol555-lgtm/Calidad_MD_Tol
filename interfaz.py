...
from base_datos import consultar, crear, editar, eliminar, guardar_varios, ErrorIntegridad, inicializar_base
        ...
        with st.expander('✏️ Editar o completar registro', expanded=True):
            ...
            if guardar:
                ...
                if missing:
                    ...
                    updates = {**values, **auto}
                    updates = {
                        campo: (None if valor is None or (isinstance(valor, str) and not valor.strip()) or (not isinstance(valor, (str, bytes)) and pd.isna(valor)) else valor.isoformat() if isinstance(valor, (date, datetime)) else valor.item() if hasattr(valor, 'item') else valor)
