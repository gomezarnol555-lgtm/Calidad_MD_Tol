...
from base_datos import consultar, crear, editar, eliminar, guardar_varios, ErrorIntegridad, reiniciar_consecutivo, inicializar_base, transaccion
        ...
        if st.button('Guardar datos cargados y calcular cumplimiento', key=key + '_guardar', type='primary'):
            ...
            try:
                for _, r in editado.iterrows():
                    cur.execute({'accion': 'crear', 'tabla': 'metas_carga_spac', 'columnas': ['tipo_entidad', 'entidad', 'periodo_tipo', 'periodo_clave', 'meta', 'actualizado_por', 'actualizado_en'], 'upsert': True, 'ignorar': False}, (tipo_entidad, str(r['Entidad']), periodo_tipo, periodo_clave, float(r['Datos cargados'] or 0), st.session_state.auth['usuario'], now_iso()))
                c.commit()
            except Exception:
                c.rollback()
                raise
            ...
            try:
                for entidad, fecha_columna, valor in cambios:
                    if valor is None:
                        ...
                        cur.execute({'accion': 'crear', 'tabla': 'ajustes_diarios_spac', 'columnas': ['tipo_entidad', 'entidad', 'fecha', 'valor', 'justificacion', 'actualizado_por', 'actualizado_en'], 'upsert': True, 'ignorar': False}, (tipo_entidad, entidad, fecha_columna, float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso()))
                c.commit()
            except Exception:
                c.rollback()
                raise
    ...
    with st.expander('Agregar nuevo usuario', expanded=False):
        ...
        if crear_usuario:
            ...
            if faltan:
                ...
                crear('usuarios', {'usuario': usuario, 'nombre': nombre, 'password_hash': hash_password(password), 'rol': nuevo_rol, 'activo': 1, 'intentos_fallidos': 0, 'requiere_cambio_pass': 1, 'creado_en': now_iso(), 'creado_por': st.session_state.auth['usuario']})
