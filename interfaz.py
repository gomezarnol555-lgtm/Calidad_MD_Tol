...
from base_datos import consultar, crear, editar, eliminar, guardar_varios, ErrorIntegridad, ErrorOperacion, reiniciar_consecutivo, inicializar_base, limpiar_cache_datos, transaccion
...
def page_catalogos():
    ...
    with tab3:
        ...
        columnas_catalogo = ['id', 'categoria', 'valor', 'orden', 'activo']
        if raw is None or not isinstance(raw, pd.DataFrame):
            raw = pd.DataFrame(columns=columnas_catalogo)
        else:
            raw = raw.reindex(columns=columnas_catalogo)
        raw['lista'] = raw['categoria'].fillna('').astype(str).map(inv)
        raw = raw.dropna(subset=['lista']).copy()
        vista = raw[['id', 'lista', 'valor', 'orden']].rename(
            columns={'id': 'ID', 'lista': 'Lista', 'valor': 'Valor', 'orden': 'Orden'}
        )
        if vista.empty:
            st.info('No hay datos generales activos. Ejecuta las semillas o agrega el primer valor desde esta pantalla.')
        ...
        with st.expander('Agregar valor', expanded=False):
            ...
            if agregar:
                ...
                elif not consultar('catalogos', ['id'], {'categoria': etiquetas[lista], 'valor': valor.strip(), 'activo': 1}, limite=1).empty:
                    st.error('El valor ya existe en la lista seleccionada.')
                else:
                    crear('catalogos', {'categoria': etiquetas[lista], 'valor': valor.strip(), 'orden': 0, 'activo': 1})
        ...
        if rid:
            ...
            with st.expander('Editar valor seleccionado', expanded=True):
                ...
                if guardar:
                    cat = etiquetas[lista]
                    val = valor.strip()
                    dup = consultar('catalogos', ['id'], {'categoria': cat, 'valor': val, 'id': {'neq': rid}, 'activo': 1}, limite=1)
                    if not val:
                        st.error('Ingresa un valor válido.')
                    elif not dup.empty:
                        st.error('Ya existe otro valor igual en la lista seleccionada.')
                    else:
                        editar('catalogos', {'categoria': cat, 'valor': val}, {'id': rid})
