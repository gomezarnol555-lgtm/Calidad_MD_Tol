        ...
        with st.expander('✏️ Editar o completar registro', expanded=True):
            ...
            if guardar:
                ...
                if missing:
                    ...
                    updates = {campo: valor for campo, valor in updates.items() if campo in columnas_tabla and campo != 'id'}
                    ...
                    try:
                        ...
                        st.error('No se pudo guardar porque otro registro ya utiliza una combinación que debe ser única.')
                    except Exception as exc:
                        st.error('No fue posible actualizar el registro. Revisa que los campos correspondan con la estructura vigente de Supabase.')
                        st.caption(str(exc))
                    else:
                        if afectados == 0:
                            st.warning('Supabase no encontró el registro o no devolvió la fila actualizada.')
                        else:
