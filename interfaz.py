    ...
    if is_dev():
        ...
        if st.button('Guardar datos cargados y calcular cumplimiento', key=key + '_guardar', type='primary'):
            for _, r in editado.iterrows():
                crear({'accion': 'crear', 'tabla': 'metas_carga_spac', 'columnas': ['tipo_entidad', 'entidad', 'periodo_tipo', 'periodo_clave', 'meta', 'actualizado_por', 'actualizado_en'], 'upsert': True, 'ignorar': False}, (tipo_entidad, str(r['Entidad']), periodo_tipo, periodo_clave, float(r['Datos cargados'] or 0), st.session_state.auth['usuario'], now_iso()))
    ...
    if st.button('Guardar ajustes diarios', key=key + '_guardar', type='primary'):
        ...
        if errores:
            ...
            for entidad, fecha_columna, valor in cambios:
                if valor is None:
                    crear({'accion': 'eliminar', 'tabla': 'ajustes_diarios_spac', 'filtros_texto': 'tipo_entidad=? AND entidad=? AND fecha=?'}, (tipo_entidad, entidad, fecha_columna))
                else:
                    crear({'accion': 'crear', 'tabla': 'ajustes_diarios_spac', 'columnas': ['tipo_entidad', 'entidad', 'fecha', 'valor', 'justificacion', 'actualizado_por', 'actualizado_en'], 'upsert': True, 'ignorar': False}, (tipo_entidad, entidad, fecha_columna, float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso()))
