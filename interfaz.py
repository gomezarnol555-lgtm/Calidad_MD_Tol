...
def editor_metas_cumplimiento(tipo_entidad, entidades, teoricos, periodo_tipo, periodo_clave, key):
    ...
    guardados = consultar({'accion': 'consultar', 'tabla': 'metas_carga_spac', 'columnas': 'entidad,meta', 'filtros_texto': 'tipo_entidad=? AND periodo_tipo=? AND periodo_clave=?', 'orden_texto': '', 'limite': None}, (tipo_entidad, periodo_tipo, periodo_clave))
    ...
    if is_dev():
        ...
        if st.button('Guardar datos cargados y calcular cumplimiento', key=key + '_guardar', type='primary'):
            for _, r in editado.iterrows():
                entidad_meta = str(r['Entidad'])
                meta = float(r['Datos cargados'] or 0)
                existente = consultar({'accion': 'consultar', 'tabla': 'metas_carga_spac', 'columnas': 'id', 'filtros_texto': 'tipo_entidad=? AND entidad=? AND periodo_tipo=? AND periodo_clave=?', 'orden_texto': '', 'limite': 1}, (tipo_entidad, entidad_meta, periodo_tipo, periodo_clave))
                if existente.empty:
                    crear({'accion': 'crear', 'tabla': 'metas_carga_spac', 'columnas': ['tipo_entidad', 'entidad', 'periodo_tipo', 'periodo_clave', 'meta', 'actualizado_por', 'actualizado_en'], 'upsert': False, 'ignorar': False}, (tipo_entidad, entidad_meta, periodo_tipo, periodo_clave, meta, st.session_state.auth['usuario'], now_iso()))
                else:
                    crear({'accion': 'editar', 'tabla': 'metas_carga_spac', 'asignaciones_texto': 'meta=?,actualizado_por=?,actualizado_en=?', 'filtros_texto': 'id=?'}, (meta, st.session_state.auth['usuario'], now_iso(), int(existente.iloc[0]['id'])))
    ...
    if st.button('Guardar ajustes diarios', key=key + '_guardar', type='primary'):
        ...
        if errores:
            ...
            for entidad, fecha_columna, valor in cambios:
                existente = consultar({'accion': 'consultar', 'tabla': 'ajustes_diarios_spac', 'columnas': 'id', 'filtros_texto': 'tipo_entidad=? AND entidad=? AND fecha=?', 'orden_texto': '', 'limite': 1}, (tipo_entidad, entidad, fecha_columna))
                if valor is None:
                    if not existente.empty:
                        crear({'accion': 'eliminar', 'tabla': 'ajustes_diarios_spac', 'filtros_texto': 'id=?'}, (int(existente.iloc[0]['id']),))
                elif existente.empty:
                    crear({'accion': 'crear', 'tabla': 'ajustes_diarios_spac', 'columnas': ['tipo_entidad', 'entidad', 'fecha', 'valor', 'justificacion', 'actualizado_por', 'actualizado_en'], 'upsert': False, 'ignorar': False}, (tipo_entidad, entidad, fecha_columna, float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso()))
                else:
                    crear({'accion': 'editar', 'tabla': 'ajustes_diarios_spac', 'asignaciones_texto': 'valor=?,justificacion=?,actualizado_por=?,actualizado_en=?', 'filtros_texto': 'id=?'}, (float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso(), int(existente.iloc[0]['id'])))
...
def matriz_entregas():
    ...
    orden_entregas = (df[['entrega_id', 'fecha']].dropna(subset=['entrega_id']).drop_duplicates('entrega_id').sort_values(['fecha', 'entrega_id'])['entrega_id'].astype(int).tolist())
    consecutivos = {entrega_id: posicion + 1 for posicion, entrega_id in enumerate(orden_entregas)}
    vista['Registro'] = vista['entrega_id'].map(lambda valor: consecutivos.get(int(valor), '') if pd.notna(valor) else '')
    vista['PDF'] = 'Disponible'
    vista = vista.drop(columns=['entrega_id']).rename(columns={'id': 'ID matriz', 'fecha': 'Fecha', 'nave': 'Nave', 'turno': 'Turno', 'analista': 'Analista', 'total_carga_datos': 'Total carga', 'horas_nave1': 'Horas Nave 1', 'horas_nave2': 'Horas Nave 2', 'horas_nave3': 'Horas Nave 3'})
    vista = vista[['ID matriz', 'Registro', 'Fecha', 'Nave', 'Turno', 'Analista', 'Total carga', 'Horas Nave 1', 'Horas Nave 2', 'Horas Nave 3', 'PDF']]
    ...
    if valida:
        ...
        if not coincidencia.empty:
            ...
            numero_visible = consecutivos.get(eid, '') if eid else ''
            st.markdown(f"### Registro seleccionado: {numero_visible or 'Sin número'} | {row['fecha']} | {row['nave']} | {row['analista']}")
            if eid:
            ...
            if is_dev() and st.session_state.get('mx_confirm') == rid:
                ...
                if x.button('Confirmar eliminación', key=f'mx_ok_{rid}'):
                    if eid:
                        crear({'accion': 'eliminar', 'tabla': 'entregas_turno_seguimientos', 'filtros_texto': 'entrega_id=?'}, (eid,))
                        crear({'accion': 'eliminar', 'tabla': 'entregas_turno_lineas', 'filtros_texto': 'entrega_id=?'}, (eid,))
                        crear({'accion': 'eliminar', 'tabla': 'entregas_turno', 'filtros_texto': 'id=?'}, (eid,))
                    crear({'accion': 'eliminar', 'tabla': 'matriz_entrega', 'filtros_texto': 'id=?'}, (rid,))
...
def page_registro():
    ...
    def form_reclamos():
        ...
        if st.button('Guardar registro', key=f'guardar_reclamos_{nonce}', type='primary'):
            ...
            duplicado = not consultar({'accion': 'consultar', 'tabla': 'reclamos_registros', 'columnas': 'id', 'filtros_texto': 'UPPER(TRIM(numero_caso))=UPPER(TRIM(?))', 'orden_texto': '', 'limite': None}, (numero_caso,)).empty if numero_caso.strip() else False
            if duplicado:
                ...
                try:
                    rid = crear({'accion': 'crear', 'tabla': 'reclamos_registros', 'columnas': ['fecha', 'codigo_defecto', 'descripcion_defecto', 'fuente', 'mercado', 'pais_estado', 'numero_caso', 'item', 'producto', 'cliente', 'familia', 'descripcion_reclamo', 'causa_raiz', 'acciones_correctivas_contingentes', 'comprobado', 'cantidad_afectada', 'unidad', 'sector', 'estado_reclamo', 'fecha_cierre', 'tsp_numero', 'caducidad', 'lote', 'nave', 'pmd', 'tipo_defecto', 'clasificacion_defecto', 'red_social', 'creado_por', 'creado_en'], 'upsert': False, 'ignorar': False}, (fecha.isoformat(), codigo, defecto, fuente.strip(), mercado.strip(), pais_estado.strip(), numero_caso.strip(), item, producto, cliente, familia, descripcion_reclamo.strip(), causa_raiz.strip(), acciones.strip(), comprobado, float(cantidad), unidad, sector.strip(), estado, fecha_cierre.isoformat() if fecha_cierre else None, tsp.strip(), caducidad.strip(), lote.strip(), nave, pmd, tipo_defecto, clasificacion, red_social, st.session_state.auth['usuario'], now_iso()))
        ...
        with st.expander('✏️ Editar o completar registro', expanded=True):
            ...
            if guardar:
                ...
                if missing:
                    ...
                    updates = {**values, **auto}
                    updates = {campo: (None if valor is None or (isinstance(valor, str) and not valor.strip()) or (not isinstance(valor, (str, bytes)) and pd.isna(valor)) else valor.isoformat() if isinstance(valor, (date, datetime)) else valor.item() if hasattr(valor, 'item') else valor) for campo, valor in updates.items()}
                    columnas_tabla = set(row_df.columns)
                    ...
                    crear({'accion': 'editar', 'tabla': f'{table_name}', 'asignaciones_texto': f'{assignments}', 'filtros_texto': 'id=?'}, tuple((updates[c] for c in update_cols)) + (selected,))
            ...
            if row_df.empty:
                ...
                with st.expander('✏️ Editar registro de Reclamo', expanded=True):
                    ...
                    if guardar:
                        ...
                        faltan = [k for k, v in req.items() if v is None or (isinstance(v, str) and (not v.strip())) or (k == 'Cantidad afectada' and float(v) <= 0)]
                        duplicado = not consultar({'accion': 'consultar', 'tabla': 'reclamos_registros', 'columnas': 'id', 'filtros_texto': 'UPPER(TRIM(numero_caso))=UPPER(TRIM(?)) AND id<>?', 'orden_texto': '', 'limite': None}, (numero_caso, selected)).empty if numero_caso.strip() else False
                        if duplicado:
...
def page_catalogos():
    ...
    with tab3:
        ...
        raw = consultar({'accion': 'consultar', 'tabla': 'catalogos', 'columnas': 'id,categoria,valor', 'filtros_texto': 'activo=1', 'orden_texto': 'categoria,valor', 'limite': None})
        ...
        with st.expander('Agregar valor', expanded=False):
            ...
            if agregar:
                ...
                elif not consultar({'accion': 'consultar', 'tabla': 'catalogos', 'columnas': 'id', 'filtros_texto': 'categoria=? AND valor=? AND activo=1', 'orden_texto': '', 'limite': None}, (etiquetas[lista], valor.strip())).empty:
                    st.error('El valor ya existe en la lista seleccionada.')
                else:
                    crear({'accion': 'crear', 'tabla': 'catalogos', 'columnas': ['categoria', 'valor', 'activo'], 'upsert': True, 'ignorar': False}, (etiquetas[lista], valor.strip()))
        ...
        if rid:
            ...
            with st.expander('Editar valor seleccionado', expanded=True):
                ...
                if guardar:
                    cat = etiquetas[lista]
                    val = valor.strip()
                    dup = consultar({'accion': 'consultar', 'tabla': 'catalogos', 'columnas': 'id', 'filtros_texto': 'categoria=? AND valor=? AND id<>? AND activo=1', 'orden_texto': '', 'limite': None}, (cat, val, rid))
                    if not val:
                        st.error('Ingresa un valor válido.')
                    elif not dup.empty:
                        st.error('Ya existe otro valor igual en la lista seleccionada.')
                    else:
                        crear({'accion': 'editar', 'tabla': 'catalogos', 'asignaciones_texto': 'categoria=?,valor=?', 'filtros_texto': 'id=?'}, (cat, val, rid))
            ...
            if st.session_state.get('cat_datos_confirmar') == rid:
                ...
                if x.button('Confirmar eliminación', key=f'cat_datos_ok_{rid}'):
                    crear({'accion': 'editar', 'tabla': 'catalogos', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (rid,))
...
def page_usuarios():
    ...
    with st.expander('Agregar nuevo usuario', expanded=False):
        ...
        if crear:
            ...
            if faltan:
                st.error('Completa los campos obligatorios: ' + ', '.join(faltan) + '.')
            elif not consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': 'id', 'filtros_texto': 'usuario=?', 'orden_texto': '', 'limite': None}, (usuario,)).empty:
                st.error('No fue posible crear la cuenta porque el nombre de usuario ya está registrado.')
            else:
                crear({'accion': 'crear', 'tabla': 'usuarios', 'columnas': ['usuario', 'nombre', 'password_hash', 'rol', 'activo', 'intentos_fallidos', 'requiere_cambio_pass', 'creado_en'], 'upsert': False, 'ignorar': False}, (usuario, nombre, hash_password(password), nuevo_rol, now_iso()))
