...
from base_datos import consultar, crear, editar, eliminar, guardar_varios, ErrorIntegridad, ErrorOperacion, reiniciar_consecutivo, inicializar_base, transaccion
...
def cambiar_password_obligatoria():
    ...
    if guardar:
        ...
        if errores:
            ...
            crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'password_hash=?,requiere_cambio_pass=0,intentos_fallidos=0', 'filtros_texto': 'usuario=?'}, (hash_password(nueva), auth['usuario']))
...
def styles(compact=False):
    st.markdown({'accion': 'recurso', 'nombre': 'estilos'}, unsafe_allow_html=True)
...
def panel_indicadores_spac_inicio(indicador='SPAC'):
    ...
    registros = consultar({'accion': 'vista', 'nombre': 'vista_spac_matriz_nave'})
...
def _panel_registros_inicio(tipo):
    ...
    d = consultar({'accion': 'consultar', 'tabla': f'{tabla}', 'columnas': '*', 'filtros_texto': '', 'orden_texto': '', 'limite': None})
    ...
    if defecto == '_codigo_defecto_panel':
        cat = consultar({'accion': 'consultar', 'tabla': 'defectos', 'columnas': 'codigo,defecto', 'filtros_texto': '', 'orden_texto': '', 'limite': None})
...
def editor_metas_cumplimiento(tipo_entidad, entidades, teoricos, periodo_tipo, periodo_clave, key):
    ...
    if is_dev():
        editado = st.data_editor(
            base, use_container_width=True, hide_index=True, key=key,
            disabled=['Entidad', 'Datos teóricos', 'Cumplimiento %'],
            column_config={
                'Datos teóricos': st.column_config.NumberColumn(format='%.2f'),
                'Datos cargados': st.column_config.NumberColumn(min_value=0.0, step=1.0, format='%.2f'),
            ...
            try:
                for _, r in editado.iterrows():
                    crear(
                        ...
                        {
                            'tipo_entidad': tipo_entidad,
                            'entidad': str(r['Entidad']).strip(),
                            'periodo_tipo': periodo_tipo,
                            'periodo_clave': periodo_clave,
                            'meta': float(r['Datos cargados'] or 0),
                            'actualizado_por': st.session_state.auth['usuario'],
                            'actualizado_en': now_iso(),
                        },
                        upsert=True,
                        on_conflict='tipo_entidad,entidad,periodo_tipo,periodo_clave',
                    )
            except ErrorIntegridad:
                st.error('No se guardaron los datos SPAC porque la clave única del periodo no está alineada. Ejecuta el archivo SQL de corrección incluido.')
                return
            except ErrorOperacion as exc:
                st.error('No se pudieron guardar los datos cargados de SPAC.')
                st.caption(str(exc))
                return
        ...
        if errores:
            ...
            try:
                for entidad, fecha_columna, valor in cambios:
                    if valor is None:
                        cur.execute({'accion': 'eliminar', 'tabla': 'ajustes_diarios_spac', 'filtros_texto': 'tipo_entidad=? AND entidad=? AND fecha=?'}, (tipo_entidad, entidad, fecha_columna))
                    else:
                        cur.execute({'accion': 'crear', 'tabla': 'ajustes_diarios_spac', 'columnas': ['tipo_entidad', 'entidad', 'fecha', 'valor', 'justificacion', 'actualizado_por', 'actualizado_en'], 'upsert': True, 'on_conflict': 'tipo_entidad,entidad,fecha', 'ignorar': False}, (tipo_entidad, entidad, fecha_columna, float(valor), 'Ajuste administrativo', st.session_state.auth['usuario'], now_iso()))
                c.commit()
            except Exception:
                c.rollback()
                raise
...
def numero_visible_entrega(entrega_id):
    """Calcula el mismo consecutivo funcional que se muestra en la matriz."""
    if not entrega_id:
        return None
    registros = consultar({
        'accion': 'consultar',
        'tabla': 'matriz_entrega',
        'columnas': 'entrega_id,fecha',
        'filtros_texto': '',
        'orden_texto': 'fecha,entrega_id',
        'limite': None,
    })
    if registros.empty or 'entrega_id' not in registros.columns:
        return None
    orden = (
        registros.dropna(subset=['entrega_id'])
        .drop_duplicates('entrega_id')
        .sort_values(['fecha', 'entrega_id'])['entrega_id']
        .astype(int)
        .tolist()
    )
    try:
        return orden.index(int(entrega_id)) + 1
    except ValueError:
        return None
...
def matriz_entregas():
    ...
    df = consultar({'accion': 'vista', 'nombre': 'vista_matriz_entregas'})
    if df.empty:
        ...
        if not coincidencia.empty:
            ...
            if is_dev() and st.session_state.get('mx_confirm') == rid:
                ...
                if x.button('Confirmar eliminación', key=f'mx_ok_{rid}'):
                    ...
                    try:
                        if eid:
                            cur.execute({'accion': 'eliminar', 'tabla': 'entregas_turno_seguimientos', 'filtros_texto': 'entrega_id=?'}, (eid,))
                            cur.execute({'accion': 'eliminar', 'tabla': 'entregas_turno_lineas', 'filtros_texto': 'entrega_id=?'}, (eid,))
                            cur.execute({'accion': 'eliminar', 'tabla': 'entregas_turno', 'filtros_texto': 'id=?'}, (eid,))
                        cur.execute({'accion': 'eliminar', 'tabla': 'matriz_entrega', 'filtros_texto': 'id=?'}, (rid,))
                        cdb.commit()
                    except Exception:
                        cdb.rollback()
                        raise
...
def page_registro():
    ...
    def form_hallazgo(tabla, titulo, audit_action):
        ...
        prod = consultar({'accion': 'consultar', 'tabla': 'productos', 'columnas': '*', 'filtros_texto': 'activo=1', 'orden_texto': 'descripcion', 'limite': None})
        defs = consultar({'accion': 'consultar', 'tabla': 'defectos', 'columnas': '*', 'filtros_texto': 'activo=1', 'orden_texto': 'CAST(codigo AS INTEGER)', 'limite': None})
        ...
        if ok:
            ...
            if faltantes:
                ...
                rid = crear({'accion': 'crear', 'tabla': f'{tabla}', 'columnas': ['dia', 'mes', 'anio', 'nave', 'linea_sector', 'familia', 'equipo_hallazgo', 'item', 'producto', 'lote', 'descripcion_hallazgo', 'tipo', 'particulas_halladas', 'accion_contingente', 'investigacion_origen', 'analista_detecta', 'supervisor_responsable', 'acciones_evitar_incidencia', 'creado_por', 'creado_en', 'etapa', 'codigo_defecto', 'semana', 'turno', 'responsable_detecta', 'acciones_inmediatas', 'disposicion', 'cantidad_observada', 'status', 'categoria_inicial', 'sector_hallazgo', 'ubicacion_material'], 'upsert': False, 'ignorar': False}, (fecha.day, fecha.month, fecha.year, nave, linea_sector, familia, equipo, item, producto, lote, descripcion, tipo, int(particulas), acciones, investigacion, analista, supervisor, evitar, st.session_state.auth['usuario'], now_iso(), '', codigo, int(semana), turno, '', acciones, '', 0.0, '', categoria, sector_hallazgo, ubicacion_material))
    ...
    def form_devoluciones():
        ...
        if st.button('Guardar registro', key=f'guardar_dev_{nonce}', type='primary'):
            ...
            if faltan:
                ...
                rid = crear({'accion': 'crear', 'tabla': 'devoluciones_registros', 'columnas': ['fecha', 'codigo_defecto', 'defecto', 'cedis', 'pais_estado', 'item', 'producto', 'cliente', 'linea', 'familia', 'lote', 'caducidad', 'descripcion_defecto', 'comprobado', 'cantidad_afectada', 'unidad', 'sector', 'disposicion', 'tsp_numero', 'status', 'nave', 'creado_por', 'creado_en'], 'upsert': False, 'ignorar': False}, (fecha.isoformat(), codigo, defecto, cedis.strip(), pais_estado.strip(), item, producto, cliente, '', familia, lote.strip(), caducidad.strip(), descripcion.strip(), comprobado, float(cantidad), unidad, sector.strip(), disposicion, tsp.strip(), status, nave, st.session_state.auth['usuario'], now_iso()))
    ...
    def form_reclamos():
        ...
        disposicion = st.selectbox('Disposición *', opt_blank(catalog('disposicion')), key=f'recl_disposicion_{nonce}')
        a, b, c = st.columns(3)
        ...
        if st.button('Guardar registro', key=f'guardar_reclamos_{nonce}', type='primary'):
            ...
            duplicado = existe_valor_normalizado('reclamos_registros', 'numero_caso', numero_caso)
            if duplicado:
                ...
                try:
                    rid = crear('reclamos_registros', {'fecha': fecha, 'codigo_defecto': codigo, 'descripcion_defecto': defecto, 'fuente': fuente.strip(), 'mercado': mercado.strip(), 'pais_estado': pais_estado.strip(), 'numero_caso': numero_caso.strip(), 'item': item, 'producto': producto, 'cliente': cliente, 'familia': familia, 'descripcion_reclamo': descripcion_reclamo.strip(), 'causa_raiz': causa_raiz.strip(), 'acciones_correctivas_contingentes': acciones.strip(), 'comprobado': comprobado, 'cantidad_afectada': float(cantidad), 'unidad': unidad, 'sector': sector.strip(), 'estado_reclamo': estado, 'fecha_cierre': fecha_cierre, 'tsp_numero': tsp.strip(), 'caducidad': caducidad.strip(), 'lote': lote.strip(), 'nave': nave, 'pmd': pmd, 'tipo_defecto': tipo_defecto, 'clasificacion_defecto': clasificacion, 'red_social': red_social, 'disposicion': disposicion, 'creado_por': st.session_state.auth['usuario'], 'creado_en': now_iso()})
...
def page_consulta():
        ...
        if st.session_state.get(f'confirm_del_{key}') == selected:
            ...
            with c1:
                if st.button('Confirmar eliminación', key=f'ok_del_{key}_{selected}'):
                    crear({'accion': 'eliminar', 'tabla': f'{table_name}', 'filtros_texto': 'id=?'}, (selected,))
    ...
    def edit_record(table_name, key, selected):
        row_df = consultar({'accion': 'consultar', 'tabla': f'{table_name}', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (selected,))
        if row_df.empty:
        ...
        with st.expander('✏️ Editar o completar registro', expanded=True):
            ...
            if guardar:
                ...
                if missing:
                    ...
                    updates = {**values, **auto}
                    columnas_disponibles = set(row_df.columns)
                    updates = {campo: valor for campo, valor in updates.items() if campo in columnas_disponibles and campo != 'id'}
                    if {'actualizado_por', 'actualizado_en'}.issubset(columnas_disponibles):
                        updates['actualizado_por'] = st.session_state.auth['usuario']
                        updates['actualizado_en'] = now_iso()
                    try:
                        afectados = editar(table_name, updates, {'id': int(selected)})
                    except ErrorIntegridad:
                        st.error('No pude guardar porque existe un registro incompatible o repetido.')
                    except Exception as exc:
                        st.error('No pude actualizar el registro en la base de datos configurada.')
                        st.caption(str(exc))
                    else:
                        if afectados < 1:
                            st.warning('No se actualizó ninguna fila. Actualiza la página y vuelve a intentarlo.')
                        else:
    ...
    with t1:
        df = consultar({'accion': 'consultar', 'tabla': 'pnc_registros', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id ASC', 'limite': None})
    ...
    with t2:
        df = consultar({'accion': 'consultar', 'tabla': 'me_registros', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id ASC', 'limite': None})
    ...
    with t3:
        df = consultar({'accion': 'consultar', 'tabla': 'ddm_rx_registros', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id ASC', 'limite': None})
        campos_actuales = ['id', 'dia', 'mes', 'anio', 'semana', 'nave', 'item', 'producto', 'familia', 'lote', 'linea_sector', 'codigo_defecto', 'turno', 'supervisor_responsable', 'analista_detecta', 'descripcion_hallazgo', 'acciones_inmediatas', 'equipo_hallazgo', 'tipo', 'particulas_halladas', 'investigacion_origen', 'acciones_evitar_incidencia', 'creado_por', 'creado_en', 'actualizado_por', 'actualizado_en']
        disponibles = [c for c in campos_actuales if c in df.columns]
        df_tabla = df[disponibles].copy()
        if all((c in df_tabla.columns for c in ['dia', 'mes', 'anio'])):
    ...
    with t4:
        df = consultar({'accion': 'consultar', 'tabla': 'reclamos_registros', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id ASC', 'limite': None})
        ...
        if selected:
            ...
            row_df = consultar({'accion': 'consultar', 'tabla': 'reclamos_registros', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (selected,))
            if row_df.empty:
                st.warning('El registro seleccionado ya no está disponible.')
            else:
                row = row_df.iloc[0].to_dict()
                ...
                with st.expander('✏️ Editar registro de Reclamo', expanded=True):
                    with st.form(f'editar_reclamo_{selected}'):
                        ...
                        disposicion_ops = opt_blank(catalog('disposicion'))
                        disposicion_actual = str(row.get('disposicion') or '')
                        if disposicion_actual and disposicion_actual not in disposicion_ops:
                            disposicion_ops.append(disposicion_actual)
                        disposicion = st.selectbox('Disposición *', disposicion_ops, index=idx_or_zero(disposicion_ops, disposicion_actual))
                        a, b, c = st.columns(3)
                    ...
                    if guardar:
                        ...
                        faltan = [k for k, v in req.items() if v is None or (isinstance(v, str) and (not v.strip())) or (k == 'Cantidad afectada' and float(v) <= 0)]
                        duplicado = existe_valor_normalizado('reclamos_registros', 'numero_caso', numero_caso, excluir_id=selected)
                        if duplicado:
                            ...
                            editar('reclamos_registros', {'fecha': fecha, 'codigo_defecto': codigo, 'descripcion_defecto': defecto, 'fuente': fuente.strip(), 'mercado': mercado, 'pais_estado': pais_estado.strip(), 'numero_caso': numero_caso.strip(), 'item': item, 'producto': producto, 'cliente': cliente, 'familia': familia, 'descripcion_reclamo': descripcion_reclamo.strip(), 'causa_raiz': causa_raiz.strip(), 'acciones_correctivas_contingentes': acciones.strip(), 'comprobado': comprobado, 'cantidad_afectada': float(cantidad), 'unidad': unidad, 'sector': sector.strip(), 'estado_reclamo': estado, 'fecha_cierre': fecha_cierre, 'tsp_numero': tsp.strip(), 'caducidad': caducidad.strip(), 'lote': lote.strip(), 'nave': nave, 'pmd': pmd, 'tipo_defecto': tipo_defecto, 'clasificacion_defecto': clasificacion, 'red_social': red_social, 'disposicion': disposicion, 'actualizado_por': st.session_state.auth['usuario'], 'actualizado_en': now_iso()}, {'id': int(selected)})
    ...
    with t5:
        df = consultar({'accion': 'consultar', 'tabla': 'devoluciones_registros', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id ASC', 'limite': None})
        ...
        if selected:
            ...
            row_df = consultar({'accion': 'consultar', 'tabla': 'devoluciones_registros', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (selected,))
            if row_df.empty:
                st.warning('El registro seleccionado ya no está disponible.')
            else:
                ...
                with st.expander('✏️ Editar registro de Devolución', expanded=True):
                    ...
                    if guardar:
                        ...
                        if faltan:
                            ...
                            crear({'accion': 'editar', 'tabla': 'devoluciones_registros', 'asignaciones_texto': 'fecha=?,codigo_defecto=?,defecto=?,cedis=?,pais_estado=?,item=?,producto=?,cliente=?,familia=?,lote=?,caducidad=?,descripcion_defecto=?,comprobado=?,cantidad_afectada=?,unidad=?,sector=?,disposicion=?,tsp_numero=?,status=?,nave=?,actualizado_por=?,actualizado_en=?', 'filtros_texto': 'id=?'}, (fecha.isoformat(), codigo, defecto, cedis.strip(), pais.strip(), item, producto, cliente, familia, lote.strip(), cad.strip(), desc.strip(), comp, float(cant), unidad, sector.strip(), disp, tsp.strip(), status, nave, st.session_state.auth['usuario'], now_iso(), selected))
...
def page_muestras_retencion():
    ...
    productos = consultar({'accion': 'consultar', 'tabla': 'productos', 'columnas': 'item,descripcion', 'filtros_texto': 'activo=1', 'orden_texto': 'descripcion', 'limite': None})
    ...
    if guardar:
        ...
        if faltan:
            ...
            rid = crear({'accion': 'crear', 'tabla': f'{tabla}', 'columnas': ['item', 'descripcion', 'lote', 'destino', 'numero_muestras', 'numero_corrugado', 'responsable', 'observaciones', 'creado_por', 'creado_en'], 'upsert': False, 'ignorar': False}, (item, descripcion, lote.strip(), destino, float(muestras), float(corrugado), responsable, observaciones.strip(), st.session_state.auth['usuario'], now_iso()))
...
def consulta_muestras_retencion():
    ...
    for tab, (tabla, nombre) in zip(tabs, configuracion):
        with tab:
            df = consultar({'accion': 'consultar', 'tabla': f'{tabla}', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id DESC', 'limite': None})
            ...
            original_df = consultar({'accion': 'consultar', 'tabla': f'{tabla}', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (rid,))
            ...
            with st.expander(f'✏️ Editar registro N° {rid}', expanded=True):
                ...
                if actualizar:
                    ...
                    if faltan:
                        ...
                        crear({'accion': 'editar', 'tabla': f'{tabla}', 'asignaciones_texto': 'item=?,descripcion=?,lote=?,destino=?,numero_muestras=?,numero_corrugado=?,responsable=?,observaciones=?,actualizado_por=?,actualizado_en=?', 'filtros_texto': 'id=?'}, (item, descripcion, lote.strip(), destino, float(muestras), float(corrugado), responsable, observaciones.strip(), st.session_state.auth['usuario'], now_iso(), rid))
            ...
            if is_dev():
                ...
                if st.session_state.get(f'confirm_{tabla}') == rid:
                    ...
                    if d1.button('Confirmar eliminación', key=f'ok_{tabla}_{rid}'):
                        crear({'accion': 'eliminar', 'tabla': f'{tabla}', 'filtros_texto': 'id=?'}, (rid,))
...
def page_entrega_turno():
    ...
    if nave in ['Nave 2', 'Nave 3']:
        ...
        if st.button('Guardar entrega de turno', type='primary', key=f'et23_g_{nave}_{n}'):
            ...
            repetida = not consultar({'accion': 'consultar', 'tabla': 'matriz_entrega', 'columnas': 'id', 'filtros_texto': 'fecha=? AND analista=?', 'orden_texto': '', 'limite': None}, (fecha.isoformat(), analista)).empty
            faltan = []
            if not analista:
            ...
            if errores_seguimiento:
                ...
                eid = crear({'accion': 'crear', 'tabla': 'entregas_turno', 'columnas': ['nave', 'fecha', 'analista', 'turno', 'referencia', 'total_carga_datos', 'total_horas_trabajadas', 'creado_por', 'creado_en'], 'upsert': False, 'ignorar': False}, (nave, fecha.isoformat(), analista, turno, referencia, total_c, total_h, st.session_state.auth['usuario'], now_iso()))
                for grupo, linea, producto, horas, carga, obs, orden, nvcat in filas_clasificadas:
                    if producto.strip() or horas or carga or obs.strip():
                        crear({'accion': 'crear', 'tabla': 'entregas_turno_lineas', 'columnas': ['entrega_id', 'grupo', 'linea', 'producto_descripcion', 'horas_trabajadas', 'carga_spac', 'observaciones', 'orden_fila', 'nave_catalogo'], 'upsert': False, 'ignorar': False}, (eid, grupo, linea, producto.strip(), horas, carga, obs.strip(), orden, nvcat))
                ...
                for j, (grupo, linea, tipo_analisis, resultado, obs, contador) in enumerate(resultados):
                    if resultado.strip() or obs.strip():
                        crear({'accion': 'crear', 'tabla': 'entregas_turno_lineas', 'columnas': ['entrega_id', 'grupo', 'linea', 'producto_descripcion', 'horas_trabajadas', 'carga_spac', 'observaciones', 'orden_fila'], 'upsert': False, 'ignorar': False}, (eid, 'ANÁLISIS ' + grupo, linea, tipo_analisis, 0, contador, (resultado + ' | ' + obs).strip(' |'), base_orden + j))
                for bloque, df, meta in seguimientos:
                    for orden, row in df.iterrows():
                        ...
                        if any((v.strip() for v in datos.values())):
                            ...
                            crear({'accion': 'crear', 'tabla': 'entregas_turno_seguimientos', 'columnas': ['entrega_id', 'bloque', 'registro_numero', 'hoja_fisica', 'carga_electronica', 'correo', 'descripcion_seguimiento', 'datos_json', 'orden_fila'], 'upsert': False, 'ignorar': False}, (eid, bloque, *legado, json.dumps(datos, ensure_ascii=False), int(orden)))
                ...
                numero_registro = numero_visible_entrega(eid) or eid
                st.success(f'Entrega de turno guardada correctamente. Número {numero_registro}')
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        return
    n = st.session_state.entrega_nonce
...
def page_catalogos():
    if not is_dev():
        ...
        productos = consultar({'accion': 'consultar', 'tabla': 'productos', 'columnas': 'id,item,descripcion,cliente,familia', 'filtros_texto': 'activo=1', 'orden_texto': 'descripcion', 'limite': None})
        ...
        with st.expander('Agregar producto', expanded=True):
            ...
            if agregar:
                ...
                existente = consultar({'accion': 'consultar', 'tabla': 'productos', 'columnas': 'id,activo', 'filtros_texto': 'item=?', 'orden_texto': '', 'limite': None}, (valores[0],)) if valores[0] else pd.DataFrame()
                if faltantes:
                    ...
                    crear({'accion': 'editar', 'tabla': 'productos', 'asignaciones_texto': 'descripcion=?,cliente=?,familia=?,activo=1', 'filtros_texto': 'id=?'}, (valores[1], valores[2], valores[3], int(existente.iloc[0].id)))
                    audit(st.session_state.auth['usuario'], 'REACTIVAR_PRODUCTO', f'ITEM {valores[0]}')
                    st.success('Producto agregado correctamente.')
                    st.rerun()
                else:
                    crear({'accion': 'crear', 'tabla': 'productos', 'columnas': ['item', 'descripcion', 'cliente', 'familia', 'activo'], 'upsert': False, 'ignorar': False}, tuple(valores))
        ...
        with st.expander(f'Agregar {titulo_singular.lower()}', expanded=False):
            ...
            if agregar:
                ...
                if obligatorios:
                    ...
                    st.rerun()
        if rid:
            actual = consultar({'accion': 'consultar', 'tabla': f'{tabla}', 'columnas': '*', 'filtros_texto': 'id=? AND activo=1', 'orden_texto': '', 'limite': None}, (rid,))
            if not actual.empty:
                r = actual.iloc[0]
                ...
                if st.session_state.get(f'cat_{clave}_confirmar') == rid:
                    ...
                    if x.button('Confirmar eliminación', key=f'cat_{clave}_ok_{rid}'):
                        crear({'accion': 'editar', 'tabla': f'{tabla}', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (rid,))
    ...
    with tab1:
        ...
        catalogo_seleccionable('productos', {'accion': 'consultar', 'tabla': 'productos', 'columnas': 'id,item,descripcion,cliente,familia', 'filtros_texto': 'activo=1', 'orden_texto': 'descripcion', 'limite': None}, {'id': 'ID', 'item': 'ITEM', 'descripcion': 'Descripción', 'cliente': 'Cliente', 'familia': 'Familia'}, 'productos', 'Producto', [('item', 'ITEM *'), ('descripcion', 'Descripción *'), ('cliente', 'Cliente *'), ('familia', 'Familia *')], {'accion': 'crear', 'tabla': 'productos', 'columnas': ['item', 'descripcion', 'cliente', 'familia', 'activo'], 'upsert': True, 'ignorar': False}, {'accion': 'editar', 'tabla': 'productos', 'asignaciones_texto': 'item=?,descripcion=?,cliente=?,familia=?', 'filtros_texto': 'id=?'}, {'accion': 'consultar', 'tabla': 'productos', 'columnas': 'id', 'filtros_texto': 'item=? AND descripcion=? AND cliente=? AND familia=? AND activo=1', 'orden_texto': '', 'limite': None})
    with tab2:
        st.subheader('Defectos')
        st.caption('Selecciona una fila para editarla o eliminarla. Los cambios se aplican automáticamente en los formularios relacionados.')
        catalogo_seleccionable('defectos', {'accion': 'consultar', 'tabla': 'defectos', 'columnas': 'id,codigo,defecto,tipo_defecto,clasificacion', 'filtros_texto': 'activo=1', 'orden_texto': 'CAST(codigo AS INTEGER)', 'limite': None}, {'id': 'ID', 'codigo': 'Código', 'defecto': 'Defecto', 'tipo_defecto': 'Tipo de defecto', 'clasificacion': 'Clasificación'}, 'defectos', 'Defecto', [('codigo', 'Código *'), ('defecto', 'Defecto *'), ('tipo_defecto', 'Tipo de defecto *'), ('clasificacion', 'Clasificación *')], {'accion': 'crear', 'tabla': 'defectos', 'columnas': ['codigo', 'defecto', 'tipo_defecto', 'clasificacion', 'activo'], 'upsert': True, 'ignorar': False}, {'accion': 'editar', 'tabla': 'defectos', 'asignaciones_texto': 'codigo=?,defecto=?,tipo_defecto=?,clasificacion=?', 'filtros_texto': 'id=?'}, {'accion': 'consultar', 'tabla': 'defectos', 'columnas': 'id', 'filtros_texto': 'codigo=? AND defecto=? AND tipo_defecto=? AND clasificacion=? AND activo=1', 'orden_texto': '', 'limite': None}, {'tipo_defecto': catalog('tipo_defecto') or ['Funcional', 'Contaminación'], 'clasificacion': ['Inocuidad', 'Calidad', 'Salubridad', 'Legalidad']})
    with tab3:
        st.subheader('Datos generales')
        st.caption('Administro listas generales desde una interfaz neutral. Los formularios consumen únicamente los valores activos de la tabla de catálogos.')
        ...
        nombres_listas = list(listas_generales)
        categoria_por_nombre = listas_generales
        nombre_por_categoria = {categoria: nombre for nombre, categoria in listas_generales.items()}

        raw = consultar(
            tabla='catalogos',
            columnas=['id', 'categoria', 'valor', 'orden', 'activo'],
            filtros={'activo': 1},
            ordenar_por=[('categoria', 'asc'), ('orden', 'asc'), ('valor', 'asc')],
        )
        if raw.empty:
            raw = pd.DataFrame(columns=['id', 'categoria', 'valor', 'orden', 'activo'])
        raw['lista'] = raw['categoria'].map(nombre_por_categoria)
        raw = raw.dropna(subset=['lista']).copy()
        vista = raw[['id', 'lista', 'valor', 'orden']].rename(
            columns={'id': 'ID', 'lista': 'Lista', 'valor': 'Valor', 'orden': 'Orden'}
        )
        ...
        evento = st.dataframe(
            ...
            key=f'cat_datos_tabla_{nonce}',
        ) if not vista.empty else None
        st.download_button(
            'Descargar tabla de datos generales',
            vista.to_csv(index=False).encode('utf-8-sig'),
            f"datos_generales_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            'text/csv',
            key='cat_datos_csv',
        )
        filas = getattr(evento, 'selection', {}).get('rows', []) if evento is not None else []
        ...
        with st.expander('Agregar valor', expanded=False):
            with st.form('cat_datos_agregar', clear_on_submit=True):
                a, b, c = st.columns([1.4, 2.4, 0.8])
                lista = a.selectbox('Lista *', nombres_listas)
                valor = b.text_input('Valor *')
                orden = c.number_input('Orden', min_value=0, step=1, value=0)
                agregar = st.form_submit_button('Agregar valor', type='primary')
            if agregar:
                categoria = categoria_por_nombre[lista]
                valor_limpio = valor.strip()
                duplicado = existe_valor_normalizado('catalogos', 'valor', valor_limpio) if categoria == 'disposicion' else False
                coincidencias = consultar(
                    tabla='catalogos',
                    columnas=['id', 'activo'],
                    filtros={'categoria': categoria, 'valor': valor_limpio},
                    limite=1,
                ) if valor_limpio else pd.DataFrame()
                if not valor_limpio:
                    st.error('Ingresa el valor que deseas agregar.')
                elif duplicado and not coincidencias.empty:
                    st.error('El valor ya existe en la lista seleccionada.')
                elif not coincidencias.empty:
                    editar(
                        'catalogos',
                        {'activo': 1, 'orden': int(orden)},
                        {'id': int(coincidencias.iloc[0]['id'])},
                    )
                    audit(st.session_state.auth['usuario'], 'REACTIVAR_DATO_GENERAL', f'{lista} | {valor_limpio}')
                    st.session_state.cat_datos_nonce = nonce + 1
                    st.rerun()
                else:
                    crear(
                        'catalogos',
                        {'categoria': categoria, 'valor': valor_limpio, 'orden': int(orden), 'activo': 1},
                    )
                    audit(st.session_state.auth['usuario'], 'AGREGAR_DATO_GENERAL', f'{lista} | {valor_limpio}')
                    st.session_state.cat_datos_nonce = nonce + 1
                    st.rerun()

        if rid:
            seleccion = raw[raw['id'] == rid]
            if not seleccion.empty:
                registro = seleccion.iloc[0]
                with st.expander('Editar valor seleccionado', expanded=True):
                    with st.form(f'cat_datos_editar_{rid}'):
                        ...
                        lista = a.selectbox(
                            'Lista *',
                            nombres_listas,
                            index=idx_or_zero(nombres_listas, str(registro['lista'])),
                        )
                        valor = b.text_input('Valor *', str(registro['valor']))
                        orden = c.number_input('Orden', min_value=0, step=1, value=int(registro.get('orden', 0) or 0))
                        guardar = st.form_submit_button('Guardar cambios', type='primary')
                    if guardar:
                        categoria = categoria_por_nombre[lista]
                        valor_limpio = valor.strip()
                        existentes = consultar(
                            tabla='catalogos',
                            columnas=['id', 'valor'],
                            filtros={'categoria': categoria, 'activo': 1},
                        )
                        repetido = any(
                            int(fila.id) != rid and normalizar_catalogo(fila.valor) == normalizar_catalogo(valor_limpio)
                            for fila in existentes.itertuples()
                        ) if not existentes.empty else False
                        if not valor_limpio:
                            ...
                            editar(
                                'catalogos',
                                {'categoria': categoria, 'valor': valor_limpio, 'orden': int(orden)},
                                {'id': rid},
                            )
                ...
                if st.session_state.get('cat_datos_confirmar') == rid:
                    st.warning('El valor dejará de estar disponible para nuevos registros. Los datos históricos conservarán el texto registrado.')
                    x, y = st.columns(2)
                    if x.button('Confirmar eliminación', key=f'cat_datos_ok_{rid}'):
                        editar('catalogos', {'activo': 0}, {'id': rid})
    ...
    with tab4:
        ...
        df_general = consultar({'accion': 'consultar', 'tabla': 'catalogo_naves_lineas', 'columnas': 'id,nave,linea,sector,orden', 'filtros_texto': 'activo=1', 'orden_texto': 'nave,orden,id', 'limite': None})
        if filtro_nave:
            ...
            with st.expander('Agregar relación general', expanded=False):
                ...
                if agregar_general:
                    ...
                    if not li or not se:
                        ...
                        duplicado = consultar({'accion': 'consultar', 'tabla': 'catalogo_naves_lineas', 'columnas': 'id', 'filtros_texto': 'nave=? AND linea_norm=? AND sector_norm=? AND activo=1', 'orden_texto': '', 'limite': None}, (nueva_nave, normalizar_catalogo(li), normalizar_catalogo(se)))
                        if not duplicado.empty:
                            st.error('La relación ya existe en el catálogo general.')
                        else:
                            orden = int(consultar({'accion': 'consultar', 'tabla': 'catalogo_naves_lineas', 'columnas': 'COALESCE(MAX(orden),-1)+1 AS n', 'filtros_texto': 'nave=?', 'orden_texto': '', 'limite': None}, (nueva_nave,)).iloc[0]['n'])
                            crear({'accion': 'crear', 'tabla': 'catalogo_naves_lineas', 'columnas': ['nave', 'linea', 'sector', 'linea_norm', 'sector_norm', 'orden', 'activo'], 'upsert': False, 'ignorar': False}, (nueva_nave, li, se, normalizar_catalogo(li), normalizar_catalogo(se), orden))
                            ...
                            st.rerun()
            if id_general:
                actual = consultar({'accion': 'consultar', 'tabla': 'catalogo_naves_lineas', 'columnas': '*', 'filtros_texto': 'id=? AND activo=1', 'orden_texto': '', 'limite': None}, (id_general,))
                if not actual.empty:
                    r = actual.iloc[0]
                    with st.expander('Editar relación general seleccionada', expanded=True):
                        ...
                        if guardar_general:
                            li = linea_e.strip()
                            se = sector_e.strip()
                            duplicado = consultar({'accion': 'consultar', 'tabla': 'catalogo_naves_lineas', 'columnas': 'id', 'filtros_texto': 'nave=? AND linea_norm=? AND sector_norm=? AND id<>? AND activo=1', 'orden_texto': '', 'limite': None}, (nave_e, normalizar_catalogo(li), normalizar_catalogo(se), id_general))
                            if not li or not se:
                                st.error('Completa Línea y Sector.')
                            elif not duplicado.empty:
                                st.error('Ya existe otra relación activa con esos datos.')
                            else:
                                crear({'accion': 'editar', 'tabla': 'catalogo_naves_lineas', 'asignaciones_texto': 'nave=?,linea=?,sector=?,linea_norm=?,sector_norm=?,orden=?', 'filtros_texto': 'id=?'}, (nave_e, li, se, normalizar_catalogo(li), normalizar_catalogo(se), int(orden_e), id_general))
                    ...
                    if st.session_state.get('cat_general_confirmar') == id_general:
                        ...
                        if x.button('Sí, eliminar', key=f'cat_general_ok_{id_general}'):
                            crear({'accion': 'editar', 'tabla': 'catalogo_naves_lineas', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (id_general,))
    ...
    with tab5:
        ...
        df = consultar({'accion': 'consultar', 'tabla': 'catalogo_formatos_entrega', 'columnas': 'id,linea,sector,tipo_analisis,orden_linea,orden_sector', 'filtros_texto': 'formato_nave=? AND tipo=? AND activo=1', 'orden_texto': 'orden_linea,orden_sector,id', 'limite': None}, (nave_fmt, tipo_fmt))
            ...
            with st.expander('Agregar elemento al formato', expanded=False):
                ...
                if agregar:
                    ...
                    if faltan:
                        ...
                        existente = consultar({'accion': 'consultar', 'tabla': 'catalogo_formatos_entrega', 'columnas': 'id', 'filtros_texto': 'formato_nave=? AND tipo=? AND linea=? AND sector=? AND tipo_analisis=?', 'orden_texto': '', 'limite': None}, (nave_fmt, tipo_fmt, linea.strip(), sector.strip(), analisis.strip() if tipo_fmt == 'ANALISIS' else ''))
                        if existente.empty:
                            crear({'accion': 'crear', 'tabla': 'catalogo_formatos_entrega', 'columnas': ['formato_nave', 'tipo', 'linea', 'sector', 'tipo_analisis', 'orden_linea', 'orden_sector', 'activo'], 'upsert': False, 'ignorar': False}, (nave_fmt, tipo_fmt, linea.strip(), sector.strip(), analisis.strip() if tipo_fmt == 'ANALISIS' else '', int(ol), int(os)))
                        else:
                            crear({'accion': 'editar', 'tabla': 'catalogo_formatos_entrega', 'asignaciones_texto': 'activo=1,orden_linea=?,orden_sector=?', 'filtros_texto': 'id=?'}, (int(ol), int(os), int(existente.iloc[0].id)))
                        ...
                        st.rerun()
            if rid:
                r = consultar({'accion': 'consultar', 'tabla': 'catalogo_formatos_entrega', 'columnas': '*', 'filtros_texto': 'id=? AND activo=1', 'orden_texto': '', 'limite': None}, (rid,))
                if not r.empty:
                    r = r.iloc[0]
                    with st.expander('Editar elemento seleccionado', expanded=True):
                        ...
                        if guardar:
                            ...
                            if not linea_editada or not sector_editado or (tipo_fmt == 'ANALISIS' and (not analisis_editado)):
                                ...
                                duplicado = consultar({'accion': 'consultar', 'tabla': 'catalogo_formatos_entrega', 'columnas': 'id', 'filtros_texto': 'formato_nave=? AND tipo=? AND linea=? AND sector=? AND tipo_analisis=? AND id<>? AND activo=1', 'orden_texto': '', 'limite': 1}, (nave_fmt, tipo_fmt, linea_editada, sector_editado, analisis_editado, rid))
                                if not duplicado.empty:
                                    st.error('No se puede guardar porque ya existe otro elemento activo con la misma Nave, Línea, Sector y Tipo de análisis.')
                                else:
                                    try:
                                        crear({'accion': 'editar', 'tabla': 'catalogo_formatos_entrega', 'asignaciones_texto': 'linea=?,sector=?,tipo_analisis=?,orden_linea=?,orden_sector=?', 'filtros_texto': 'id=?'}, (linea_editada, sector_editado, analisis_editado, int(ol), int(os), rid))
                                    except ErrorIntegridad:
                    ...
                    if st.session_state.get('fmt_confirm') == rid:
                        ...
                        if x.button('Sí, eliminar', key=f'fmt_ok_{rid}'):
                            crear({'accion': 'editar', 'tabla': 'catalogo_formatos_entrega', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (rid,))
        ...
        seg = consultar({'accion': 'consultar', 'tabla': 'catalogo_seguimientos_entrega', 'columnas': 'id,nombre,orden', 'filtros_texto': 'activo=1', 'orden_texto': 'orden,id', 'limite': None})
        ...
        with st.expander('Agregar seguimiento'):
            ...
            if alta:
                nombre = nom.strip()
                ex = consultar({'accion': 'consultar', 'tabla': 'catalogo_seguimientos_entrega', 'columnas': 'id', 'filtros_texto': 'UPPER(TRIM(nombre))=UPPER(TRIM(?))', 'orden_texto': '', 'limite': None}, (nombre,)) if nombre else pd.DataFrame()
                if not nombre:
                    st.error('Ingresa el seguimiento.')
                elif not ex.empty:
                    crear({'accion': 'editar', 'tabla': 'catalogo_seguimientos_entrega', 'asignaciones_texto': 'nombre=?,orden=?,activo=1', 'filtros_texto': 'id=?'}, (nombre, int(orden), int(ex.iloc[0].id)))
                    audit(st.session_state.auth['usuario'], 'REACTIVAR_SEGUIMIENTO_ENTREGA', nombre)
                    st.session_state.seg_nonce = ns + 1
                    st.rerun()
                else:
                    crear({'accion': 'crear', 'tabla': 'catalogo_seguimientos_entrega', 'columnas': ['nombre', 'orden', 'activo'], 'upsert': False, 'ignorar': False}, (nombre, int(orden)))
                    audit(st.session_state.auth['usuario'], 'AGREGAR_SEGUIMIENTO_ENTREGA', nombre)
                    st.session_state.seg_nonce = ns + 1
                    st.rerun()
        if sid:
            rr = consultar({'accion': 'consultar', 'tabla': 'catalogo_seguimientos_entrega', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (sid,)).iloc[0]
            ...
            campos_seg = consultar({'accion': 'consultar', 'tabla': 'catalogo_seguimientos_campos', 'columnas': 'id,nombre,tipo_campo,opciones,obligatorio,orden', 'filtros_texto': 'seguimiento_id=? AND activo=1', 'orden_texto': 'orden,id', 'limite': None}, (sid,))
            ...
            with st.expander('Agregar campo al seguimiento'):
                ...
                if ca:
                    ...
                    if not nombre_campo:
                        ...
                        crear({'accion': 'crear', 'tabla': 'catalogo_seguimientos_campos', 'columnas': ['seguimiento_id', 'nombre', 'tipo_campo', 'opciones', 'obligatorio', 'orden', 'activo'], 'upsert': True, 'ignorar': False}, (sid, nombre_campo, ct, opciones_campo, int(cob), int(cord)))
            ...
            if cid:
                ...
                with st.expander('Editar campo seleccionado', expanded=True):
                    ...
                    if ceg:
                        ...
                        if not cen.strip():
                            ...
                            crear({'accion': 'editar', 'tabla': 'catalogo_seguimientos_campos', 'asignaciones_texto': 'nombre=?,tipo_campo=?,opciones=?,obligatorio=?,orden=?', 'filtros_texto': 'id=?'}, (cen.strip(), cet, opciones_edit, int(ceob), int(ceord), cid))
                ...
                if st.button('Eliminar campo seleccionado', key=f'seg_campo_del_{cid}'):
                    crear({'accion': 'editar', 'tabla': 'catalogo_seguimientos_campos', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (cid,))
            ...
            with st.expander('Editar seguimiento seleccionado', expanded=True):
                ...
                if guardar and ne.strip():
                    crear({'accion': 'editar', 'tabla': 'catalogo_seguimientos_entrega', 'asignaciones_texto': 'nombre=?,orden=?', 'filtros_texto': 'id=?'}, (ne.strip(), int(oe), sid))
            ...
            if st.session_state.get('seg_confirm') == sid:
                ...
                if a.button('Confirmar eliminación', key=f'seg_ok_{sid}'):
                    crear({'accion': 'editar', 'tabla': 'catalogo_seguimientos_entrega', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (sid,))
...
def page_usuarios():
    ...
    bloqueados = consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': 'id,usuario,nombre,rol,COALESCE(intentos_fallidos,0) AS intentos_fallidos', 'filtros_texto': 'activo=0', 'orden_texto': 'intentos_fallidos DESC,usuario', 'limite': None})
    with st.expander(f'🔓 Habilitación de cuentas bloqueadas ({len(bloqueados)})', expanded=not bloqueados.empty):
        if bloqueados.empty:
            ...
            if st.button('Reactivar cuenta y reiniciar intentos', type='primary', key='reactivar_usuario_bloqueado'):
                ...
                crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'activo=1,intentos_fallidos=0', 'filtros_texto': 'id=?'}, (bid,))
    ...
    with st.expander('Agregar nuevo usuario', expanded=False):
        ...
        if crear_usuario:
            ...
            if faltan:
                st.error('Completa los campos obligatorios: ' + ', '.join(faltan) + '.')
            elif existe_valor_normalizado('usuarios', 'usuario', usuario):
                ...
                try:
                    crear(
                        'usuarios',
                        {
                            ...
                            'creado_en': now_iso(),
                            'creado_por': st.session_state.auth['usuario'],
                        },
                    )
                except ErrorIntegridad:
                    st.error('No fue posible crear la cuenta porque el nombre de usuario ya está registrado.')
                except ErrorOperacion as exc:
                    st.error('No fue posible crear el usuario. Ejecuta el SQL de corrección incluido y vuelve a intentarlo.')
                    st.caption(str(exc))
                else:
    ...
    usuarios = consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': "id,usuario,nombre,rol,COALESCE(intentos_fallidos,0) AS intentos_fallidos,COALESCE(requiere_cambio_pass,0) AS requiere_cambio_pass,CASE WHEN activo=1 THEN 'Habilitado' ELSE 'Bloqueado / Inhabilitado' END AS estado,creado_en", 'filtros_texto': '', 'orden_texto': 'id', 'limite': None})
    if usuarios.empty:
    ...
    actual_df = consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': '*', 'filtros_texto': 'id=?', 'orden_texto': '', 'limite': None}, (rid,))
    if actual_df.empty:
        st.session_state.usuarios_nonce += 1
        st.rerun()
    r = actual_df.iloc[0]
    ...
    with st.expander('Editar información y rol', expanded=True):
        ...
        if guardar:
            ...
            if not usuario or not nombre:
            ...
            elif not consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': 'id', 'filtros_texto': 'usuario=? AND id<>?', 'orden_texto': '', 'limite': None}, (usuario, rid)).empty:
                ...
                if nueva:
                    crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'usuario=?,nombre=?,rol=?,password_hash=?,requiere_cambio_pass=1,intentos_fallidos=0', 'filtros_texto': 'id=?'}, (usuario, nombre, rol_editado, hash_password(nueva), rid))
                else:
                    crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'usuario=?,nombre=?,rol=?', 'filtros_texto': 'id=?'}, (usuario, nombre, rol_editado, rid))
                audit(st.session_state.auth['usuario'], 'EDITAR_USUARIO', f'ID {rid} | Usuario {usuario} | Rol {rol_editado}')
                if rid == int(consultar({'accion': 'consultar', 'tabla': 'usuarios', 'columnas': 'id', 'filtros_texto': 'usuario=?', 'orden_texto': '', 'limite': None}, (st.session_state.auth['usuario'],)).iloc[0].id):
    ...
    if inhabilitar:
        if str(r['usuario']) == st.session_state.auth['usuario']:
            ...
            crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'activo=0', 'filtros_texto': 'id=?'}, (rid,))
            audit(st.session_state.auth['usuario'], 'INHABILITAR_USUARIO', f"ID {rid} | {r['usuario']}")
            st.session_state.usuarios_nonce += 1
            st.success('Usuario inhabilitado correctamente.')
            st.rerun()
    if habilitar:
        crear({'accion': 'editar', 'tabla': 'usuarios', 'asignaciones_texto': 'activo=1,intentos_fallidos=0', 'filtros_texto': 'id=?'}, (rid,))
    ...
    if st.session_state.get('usuario_confirmar_eliminacion') == rid:
        ...
        if x.button('Confirmar eliminación', key=f'confirmar_usuario_{rid}', use_container_width=True):
            if str(r['usuario']) == st.session_state.auth['usuario']:
                ...
                crear({'accion': 'eliminar', 'tabla': 'usuarios', 'filtros_texto': 'id=?'}, (rid,))
...
def page_auditoria():
    st.title('Auditoría')
    st.dataframe(consultar({'accion': 'consultar', 'tabla': 'auditoria', 'columnas': '*', 'filtros_texto': '', 'orden_texto': 'id DESC', 'limite': 1000}), use_container_width=True, hide_index=True)
