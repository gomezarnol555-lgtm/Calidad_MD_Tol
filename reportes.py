...
def pdf_pnc(rid):
    ...
    try:
        ...
        return None
    df = read_df(sentencia_datos('REPORTES_0022'), (rid,))
    if df.empty:
        return None
    r = df.iloc[0].to_dict()
...
def pdf_pnc_fisico(rid):
    ...
    try:
        ...
        return None
    df = read_df(sentencia_datos('REPORTES_0023'), (rid,))
    if df.empty:
        return None
    r = df.iloc[0].to_dict()
...
def _pdf_hallazgo_base(tabla, rid, tipo_formato):
    ...
    try:
        ...
        return None
    df = read_df(sentencia_datos('REPORTES_0024', tabla), (rid,))
    if df.empty:
        return None
    r = df.iloc[0].to_dict()
...
def pdf_entrega(eid):
    try:
        ...
        return None
    h = read_df(sentencia_datos('REPORTES_0025'), (eid,))
    d = read_df(sentencia_datos('REPORTES_0026'), (eid,))
    m = read_df(sentencia_datos('REPORTES_0027'), (eid,))
    sg = read_df(sentencia_datos('REPORTES_0028'), (eid,))
    if h.empty:
        return None
    r = h.iloc[0]
    b = BytesIO()
