...
def pdf_entrega(eid):
    try:
        ...
        return None
    h = read_df('SELECT * FROM entregas_turno WHERE id=?', (eid,))
    d = read_df('SELECT * FROM entregas_turno_lineas WHERE entrega_id=? ORDER BY orden_fila', (eid,))
    m = read_df('SELECT * FROM matriz_entrega WHERE entrega_id=?', (eid,))
    sg = read_df('SELECT * FROM entregas_turno_seguimientos WHERE entrega_id=? ORDER BY bloque,orden_fila,id', (eid,))
    if h.empty:
        return None
    r = h.iloc[0]
    b = BytesIO()
