...
from operaciones import OPERACIONES_TABLA
...
def operacion_tabla(nombre,*recursos):
    op=dict(OPERACIONES_TABLA[str(nombre)])
    if recursos:
        for i,v in enumerate(recursos):
            for campo in ('tabla','nombre','columnas','filtros_texto','orden_texto','asignaciones_texto'):
                if isinstance(op.get(campo),str): op[campo]=op[campo].replace('{'+str(i)+'}',str(v))
    return op
...
def _ejecutar_descriptor(op,parametros,espera='consulta'):
    ...
    a=op['accion']
    if a=='recurso': return ESTILOS_APP
