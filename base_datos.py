...
class ErrorBaseDatos(Exception):
    """Error general que expongo al resto de mi aplicacion."""


class ErrorConexion(ErrorBaseDatos):
    """No pude establecer o validar la conexion configurada."""


class ErrorIntegridad(ErrorBaseDatos):
    """La operacion viola una regla de integridad o unicidad."""


class ErrorPermisos(ErrorBaseDatos):
    """El proveedor rechazo la operacion por permisos."""


class ErrorOperacion(ErrorBaseDatos):
    """El proveedor no pudo completar una operacion de datos."""
...
def diagnostico_conexion():
    try:
        ...
        return False, "La base de datos rechazo la validacion: " + str(exc)[:500]
...
def crear(tabla=None, datos=None, upsert=False, on_conflict=None):
    ...
    try:
        ...
        bajo = texto.lower()
        if any(x in bajo for x in ("duplicate", "unique", "23505")):
            raise ErrorIntegridad(texto) from exc
        if any(x in bajo for x in ("permission", "denied", "42501", "401", "403")):
            raise ErrorPermisos(texto) from exc
...
def editar(tabla, datos=None, filtros=None):
    ...
    datos = _normalizar_datos(datos or {})
    if not datos:
        return 0
    try:
...
def crear_varios(tabla, filas, upsert=False, on_conflict=None):
    if not filas:
        return []
    filas = [_normalizar_datos(fila) for fila in filas]
...
def proveedor_actual():
    """Informo el adaptador activo sin obligar a otros modulos a conocerlo."""
    return "supabase"


def maximo(tabla, columna, filtros=None, predeterminado=None):
    """Obtengo un maximo mediante el contrato publico, sin SQL en la interfaz."""
    datos = consultar(tabla=tabla, columnas=[columna], filtros=filtros or {})
    if datos.empty or columna not in datos.columns:
        return predeterminado
    serie = pd.to_numeric(datos[columna], errors="coerce").dropna()
    return predeterminado if serie.empty else serie.max().item()

class _ResultadoCompatibilidad:
    """Mantengo temporalmente compatibilidad con bloques declarativos antiguos."""
...
class _TransaccionCompatibilidad:
    """Expongo una unidad de trabajo neutral mientras migro bloques antiguos."""
    def __init__(self):
        self._cerrada = False
    ...
    def execute(self, operacion, parametros=()):
        if self._cerrada:
            raise ErrorOperacion("La unidad de trabajo ya esta cerrada.")
        if isinstance(operacion, dict) and operacion.get("accion") == "columnas":
            filas = [(None, nombre) for nombre in sorted(columnas_tabla(operacion["tabla"]))]
            return _ResultadoCompatibilidad(filas)
        resultado = _ejecutar_descriptor(operacion, parametros, "cambio")
        return _ResultadoCompatibilidad(resultado if isinstance(resultado, list) else [])
...
def transaccion():
    """Creo una unidad de trabajo mediante el contrato general de la aplicacion."""
    return _TransaccionCompatibilidad()
