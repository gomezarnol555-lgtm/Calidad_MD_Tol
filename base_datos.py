...
def _detalle_error_proveedor(exc):
    """Entrega un mensaje util sin exponer credenciales ni encabezados."""
    datos = getattr(exc, "message", None)
    if datos:
        return str(datos)[:700]
    args = getattr(exc, "args", ())
    if args and isinstance(args[0], dict):
        detalle = args[0]
        return " | ".join(
            str(detalle.get(k)) for k in ("code", "message", "details", "hint")
            if detalle.get(k)
        )[:700]
    return str(exc)[:700]
