...
def _descongelar(valor):
    if isinstance(valor, tuple):
        if all(isinstance(x, tuple) and len(x) == 2 for x in valor):
            return {k: _descongelar(v) for k, v in valor}
...
def _consultar_cache(tabla, columnas, filtros_congelados, orden_congelado, limite):
    filtros = _descongelar(filtros_congelados) if filtros_congelados else {}
    orden = _descongelar(orden_congelado) if orden_congelado else []
