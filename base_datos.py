...
def consultar(tabla=None, columnas="*", filtros=None, ordenar_por=None, limite=None, uno=False, operacion=None, parametros=()):
    ...
    # Conservo el contrato de columnas aunque la consulta no devuelva filas.
    # La interfaz nunca debe depender de que existan datos para conocer el esquema.
    if data:
        return pd.DataFrame(data)
    if columnas_limpias == "*":
        return pd.DataFrame()
    nombres = []
    for expresion in _separar_expresiones(columnas_limpias):
        nombre = expresion.split(":", 1)[0].strip()
...
def _valor_json(valor):
    """Convierte valores de pandas, NumPy y Python a tipos aceptados por PostgREST."""
    if valor is None:
        return None
    ...
    if isinstance(valor, Decimal):
        return float(valor)
    if isinstance(valor, dict):
        return {str(k): _valor_json(v) for k, v in valor.items()}
    if isinstance(valor, (list, tuple, set)):
...
def _normalizar_datos(datos):
    if not isinstance(datos, dict):
        raise TypeError("Los datos enviados a la base deben ser un diccionario.")
    return {str(campo): _valor_json(valor) for campo, valor in datos.items()}


def _detalle_error_proveedor(exc):
    """Extrae el detalle de APIError sin exponer credenciales ni encabezados."""
    detalle = None
    ...
    return str(mensaje or exc)[:700]


@st.cache_data(ttl=300, show_spinner=False)
def _tipos_columnas(tabla):
    """Consulto tipos reales; uso reglas seguras si el proveedor no los expone."""
    try:
        filas = cliente().rpc("estructura_tabla", {"nombre_tabla": str(tabla)}).execute().data or []
        return {str(x.get("nombre_columna")): str(x.get("tipo_dato") or "").lower() for x in filas}
    except Exception:
        return {}


def _preparar_datos_tabla(tabla, datos):
    """Evito enviar cadenas vacias a columnas de fecha o numericas."""
    salida = _normalizar_datos(datos or {})
    tipos = _tipos_columnas(tabla)
    fechas = {"fecha","fecha_apertura","fecha_cierre","fecha_final_tratamiento","creado_en","actualizado_en","ultimo_acceso","fecha_hora"}
    numeros = {"id","dia","mes","anio","semana","orden","orden_fila","particulas_halladas","cantidad_observada","cantidad_reproceso","cantidad_decomiso","cantidad_aprobado_segunda","cantidad_total_pnc","cantidad_afectada","numero_muestras","numero_corrugado","total_carga_datos","total_horas_trabajadas","horas_trabajadas","carga_spac","horas_nave1","horas_nave2","horas_nave3","valor","meta","intentos_fallidos","requiere_cambio_pass","activo","obligatorio","orden_linea","orden_sector"}
    for campo, valor in list(salida.items()):
        tipo = tipos.get(campo, "")
        vacio = isinstance(valor, str) and not valor.strip()
        es_fecha = campo in fechas or "date" in tipo or "timestamp" in tipo
        es_numero = campo in numeros or any(x in tipo for x in ("integer","numeric","decimal","real","double"))
        if vacio and (es_fecha or es_numero):
            salida[campo] = None
    return salida
...
def crear(tabla=None, datos=None, upsert=False, on_conflict=None):
    ...
    try:
        datos = _preparar_datos_tabla(tabla, datos)
        ...
        texto = _detalle_error_proveedor(exc)
        if any(x in texto.lower() for x in ("duplicate", "unique", "23505")):
            raise ErrorIntegridad(texto) from exc
        raise ErrorOperacion(f"No pude crear el registro en '{tabla}'. {texto}") from exc
...
def editar(tabla, datos=None, filtros=None):
    ...
    datos = _preparar_datos_tabla(tabla, datos or {})
    if not datos:
        return 0
    try:
        query = cliente().table(str(tabla)).update(datos)
        ...
        if any(x in texto.lower() for x in ("duplicate", "unique", "23505")):
            raise ErrorIntegridad(texto) from exc
        raise RuntimeError(
            f"No fue posible actualizar '{tabla}'. Detalle del proveedor: {texto}"
        ) from exc
...
def crear_varios(tabla, filas, upsert=False, on_conflict=None):
    if not filas:
        return []
    filas = [_preparar_datos_tabla(tabla, fila) for fila in filas]
...
def _ejecutar_descriptor(op, parametros=(), espera="consulta"):
    ...
    if accion == "crear":
        return crear(
            op["tabla"],
            dict(zip(op.get("columnas", []), parametros)),
            op.get("upsert", False),
            op.get("on_conflict"),
        )
    if accion == "editar":
...
class _ResultadoUnidadTrabajo:
    def __init__(self, filas=None): self._filas = filas or []
    def fetchall(self): return self._filas

class _UnidadTrabajo:
    """Mantengo una interfaz neutral para operaciones masivas existentes."""
    def cursor(self): return self
    def execute(self, operacion, parametros=()):
        if isinstance(operacion, dict) and operacion.get("accion") == "columnas":
            return _ResultadoUnidadTrabajo([(None, x) for x in sorted(columnas_tabla(operacion["tabla"]))])
        resultado = _ejecutar_descriptor(operacion, parametros, "cambio")
        return _ResultadoUnidadTrabajo(resultado if isinstance(resultado, list) else [])
    def commit(self): limpiar_cache_datos()
    def rollback(self): limpiar_cache_datos()
    def close(self): return None

def transaccion(): return _UnidadTrabajo()
