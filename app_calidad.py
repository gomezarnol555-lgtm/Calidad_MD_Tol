...
def formato_entrega(nave,tipo='PROCESO'):
    df=read_df("SELECT grupo,linea,tipo_analisis,orden_grupo,orden_linea FROM catalogo_formatos_entrega WHERE formato_nave=? AND tipo=? AND activo=1 ORDER BY orden_grupo,orden_linea,id",(nave,tipo))
    if tipo=='ANALISIS':
        return [(str(r.grupo),str(r.linea),str(r.tipo_analisis or '')) for r in df.itertuples()]
    grupos={}
    for r in df.itertuples(): grupos.setdefault(str(r.grupo),[]).append(str(r.linea))
    return grupos
...
def init_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS catalogo_formatos_entrega(id INTEGER PRIMARY KEY AUTOINCREMENT,formato_nave TEXT NOT NULL,tipo TEXT NOT NULL,grupo TEXT NOT NULL,linea TEXT NOT NULL,tipo_analisis TEXT DEFAULT '',orden_grupo INTEGER DEFAULT 0,orden_linea INTEGER DEFAULT 0,activo INTEGER DEFAULT 1,UNIQUE(formato_nave,tipo,grupo,linea,tipo_analisis))")
    ...
    # Semillas de catálogos: se insertan una sola vez y después respetan todos los cambios administrativos.
    if not cur.execute("SELECT 1 FROM app_config WHERE clave='semillas_base_v2'").fetchone():
        ...
        if cur.execute('SELECT COUNT(*) FROM catalogos').fetchone()[0]==0:
            for cat,values in SEED_CATALOGS.items():
                for value in values: cur.execute("INSERT OR IGNORE INTO catalogos(categoria,valor,activo) VALUES(?,?,1)",(cat,value))
        ...
        cur.execute("INSERT OR REPLACE INTO app_config(clave,valor,actualizado_en) VALUES('semillas_base_v2','1',?)",(now_iso(),))
    if not cur.execute("SELECT 1 FROM app_config WHERE clave='formatos_entrega_v1'").fetchone():
        if cur.execute('SELECT COUNT(*) FROM catalogo_formatos_entrega').fetchone()[0]==0:
            for nv,tipo,grupo,linea,analisis,og,ol in SEED_FORMATOS_ENTREGA:
                cur.execute('INSERT OR IGNORE INTO catalogo_formatos_entrega(formato_nave,tipo,grupo,linea,tipo_analisis,orden_grupo,orden_linea,activo) VALUES(?,?,?,?,?,?,?,1)',(nv,tipo,grupo,linea,analisis,og,ol))
        cur.execute("INSERT OR REPLACE INTO app_config(clave,valor,actualizado_en) VALUES('formatos_entrega_v1','1',?)",(now_iso(),))
    c.commit(); c.close()
...
def page_consulta():
    if 'consulta_tipo' not in st.session_state: st.session_state.consulta_tipo=None
    if st.session_state.consulta_tipo is None:
        ...
        opciones=[('NO_CONFORMIDADES','📋  Consulta y seguimiento de No Conformidades\n\nPNC, Materia Extraña y Detector de metales/RX.\n\nAbrir sección'),('MUESTRAS','🧪  Muestras de retención\n\nConsulta, edición y descarga de muestras.\n\nAbrir sección'),('MATRIZ','📊  Matriz entrega de turno\n\nRegistros de las tres naves, PDF y matriz histórica.\n\nAbrir sección')]
        for col,(valor,texto) in zip(st.columns(3,gap='large'),opciones):
            with col:
                st.markdown('<span class="registro-card-slot"></span>',unsafe_allow_html=True)
                if st.button(texto,key=f'consulta_{valor}'): st.session_state.consulta_tipo=valor;st.rerun()
        return
    if st.button('← Volver a Consulta y descarga',key='consulta_volver'):st.session_state.consulta_tipo=None;st.rerun()
        ...
        with st.expander('✏️ Editar o completar registro',expanded=True):
            ...
            if guardar:
                ...
                if missing: st.error('Completa los siguientes campos obligatorios: '+', '.join(dict.fromkeys(missing))+'.')
                    ...
                    st.rerun()
    if tipo_consulta=='MUESTRAS': consulta_muestras_retencion(); return
    if tipo_consulta=='MATRIZ': matriz_entregas(); return
    ...
    if nave in ['Nave 2','Nave 3']:
        ...
        for gi,(grupo,lineas) in enumerate(procesos.items()):
            ...
            for li,linea in enumerate(lineas):
                ...
                cols[0].text_input('Línea',linea,disabled=True,key=f'et23_l_{nave}_{gi}_{li}_{n}',label_visibility='collapsed')
...
def page_catalogos():
    ...
    with tab4:
        ...
        st.caption('Administra por separado la estructura visible de Nave 1, Nave 2 y Nave 3. La clasificación general de horas continúa usando el catálogo de naves, líneas y sectores.')
        nave_formato=st.radio('Formato',['Nave 1','Nave 2','Nave 3'],horizontal=True,key='fmt_nave')
        tipo_formato=st.radio('Sección',['PROCESO','ANALISIS'],horizontal=True,key='fmt_tipo')
        df=read_df('SELECT id,grupo,linea,tipo_analisis,orden_grupo,orden_linea FROM catalogo_formatos_entrega WHERE formato_nave=? AND tipo=? AND activo=1 ORDER BY orden_grupo,orden_linea,id',(nave_formato,tipo_formato))
        vista=df.rename(columns={'id':'ID','grupo':'Grupo / sector','linea':'Línea','tipo_analisis':'Tipo de análisis','orden_grupo':'Orden grupo','orden_linea':'Orden línea'})
        nonce=st.session_state.get('fmt_nonce',0)
        ev=st.dataframe(vista,use_container_width=True,hide_index=True,on_select='rerun',selection_mode='single-row',key=f'fmt_tabla_{nave_formato}_{tipo_formato}_{nonce}') if not vista.empty else None
        rows=getattr(ev,'selection',{}).get('rows',[]) if ev is not None else []
        valido=bool(rows) and isinstance(rows[0],int) and 0<=rows[0]<len(vista)
        rid=int(vista.iloc[rows[0]]['ID']) if valido else None
        if admin_required():
            with st.expander('Agregar línea, sector o análisis',expanded=False):
                with st.form(f'fmt_add_{nave_formato}_{tipo_formato}',clear_on_submit=True):
                    a,b=st.columns(2);grupo=a.text_input('Grupo / sector *');linea=b.text_input('Línea *')
                    c,d,e=st.columns(3);analisis=c.text_input('Tipo de análisis',disabled=tipo_formato!='ANALISIS');og=d.number_input('Orden del grupo',0,step=1);ol=e.number_input('Orden de la línea',0,step=1);add_ok=st.form_submit_button('Agregar al formato',type='primary')
                if add_ok:
                    if not grupo.strip() or not linea.strip(): st.error('Completa Grupo / sector y Línea.')
                    else:
                        exec_sql('INSERT OR IGNORE INTO catalogo_formatos_entrega(formato_nave,tipo,grupo,linea,tipo_analisis,orden_grupo,orden_linea,activo) VALUES(?,?,?,?,?,?,?,1)',(nave_formato,tipo_formato,grupo.strip(),linea.strip(),analisis.strip() if tipo_formato=='ANALISIS' else '',int(og),int(ol)));audit(st.session_state.auth['usuario'],'AGREGAR_FORMATO_ENTREGA',f'{nave_formato} | {tipo_formato} | {grupo} | {linea}');st.session_state.fmt_nonce=nonce+1;st.rerun()
            if rid:
                row=read_df('SELECT * FROM catalogo_formatos_entrega WHERE id=? AND activo=1',(rid,))
                if not row.empty:
                    r=row.iloc[0]
                    with st.expander('Editar elemento seleccionado',expanded=True):
                        with st.form(f'fmt_edit_{rid}'):
                            a,b=st.columns(2);grupo=a.text_input('Grupo / sector *',str(r.grupo));linea=b.text_input('Línea *',str(r.linea));c,d,e=st.columns(3);analisis=c.text_input('Tipo de análisis',str(r.tipo_analisis or ''),disabled=tipo_formato!='ANALISIS');og=d.number_input('Orden del grupo',0,value=int(r.orden_grupo));ol=e.number_input('Orden de la línea',0,value=int(r.orden_linea));save=st.form_submit_button('Guardar cambios',type='primary')
                        if save:
                            exec_sql('UPDATE catalogo_formatos_entrega SET grupo=?,linea=?,tipo_analisis=?,orden_grupo=?,orden_linea=? WHERE id=?',(grupo.strip(),linea.strip(),analisis.strip() if tipo_formato=='ANALISIS' else '',int(og),int(ol),rid));audit(st.session_state.auth['usuario'],'EDITAR_FORMATO_ENTREGA',f'ID {rid} | {nave_formato}');st.session_state.fmt_nonce=nonce+1;st.rerun()
                    ...
                    if st.session_state.get('fmt_confirm')==rid:
                        st.warning('El elemento dejará de aparecer en este formato de entrega de turno.')
                        x,y=st.columns(2)
