...
def page_registro():
    ...
    with st.form("registro"):
        ...
        sup = c4.selectbox("Supervisor", catalog("supervisor"))
        ana = c4.selectbox("Analista", catalog("analista"))
        resp = c5.selectbox("Responsable detecta", catalog("responsable_detecta"))
        disp = c5.selectbox("Disposición", catalog("disposicion"))
    ...
    if ok:
        ...
        rid = exec_sql(
            ...
            (folio, fecha.isoformat(), linea, nave, item, desc, cliente, familia, lote, etapa, cod, defecto, tipo, clas, turno, sup, ana, resp, descripcion, acciones, disp, obs, rep, dec, apr, total, status, fecha_final.isoformat() if fecha_final else None, notas, mat, st.session_state.auth["usuario"], now_iso()),
        )
