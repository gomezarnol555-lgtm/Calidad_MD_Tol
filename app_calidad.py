...
def tabla_diaria_editable(tabla,tipo_entidad,columna_entidad,key):
    """Tabla única: consulta visual y edición directa para desarrolladores."""
    ...
    st.caption('Edición directa habilitada. Captura 0 para vacaciones, incapacidad o día no laborable. Los espacios vacíos representan registros pendientes.')
...
def datos_grafica_cumplimiento(tipo_entidad,periodo_tipo):
    ...
    for fila in cargados.itertuples():
        ...
        if tipo_entidad=='ANALISTA':
            ...
            registros_calidad=read_df("SELECT m.fecha,m.total_carga_datos,COALESCE(e.nave,'') AS nave FROM matriz_entrega m LEFT JOIN entregas_turno e ON e.id=m.entrega_id")
            registros_calidad['fecha_dt']=pd.to_datetime(registros_calidad['fecha'],errors='coerce').dt.date
            serie=registros_calidad[(registros_calidad['fecha_dt']>=inicio)&(registros_calidad['fecha_dt']<=fin)&(registros_calidad['nave'].astype(str)==entidad)].groupby('fecha')['total_carga_datos'].sum().reindex(fechas)
        else:
...
def matriz_entregas():
    ...
    st.caption('Suma el TOTAL DE CARGA DE DATOS según la plantilla seleccionada por el analista: Nave 1, Nave 2 o Nave 3.')
    ...
    with cn:
        st.markdown('### Cumplimiento SPAC Producción'); editor_metas_cumplimiento('NAVE',['Nave 1','Nave 2','Nave 3'],teoricos_n,pt,pc,f'meta_n_{pt}_{pc}')
    ...
    st.caption('Visualización en líneas. La selección Semana o Mes aplicada arriba controla las tres gráficas.')
    ...
    if grafica_analistas.empty or grafica_analistas['Cumplimiento %'].dropna().empty:
        ...
        if vista_ga.empty: st.info('Selecciona al menos un analista.')
        else:
    ...
    st.markdown('### Cumplimiento SPAC Calidad')
    if grafica_calidad.empty or grafica_calidad['Cumplimiento %'].dropna().empty:
        st.info('Ingresa y guarda Datos cargados de Calidad para visualizar la gráfica.')
    else:
        st.line_chart(grafica_calidad.pivot_table(index='Periodo',columns='Entidad',values='Cumplimiento %',aggfunc='first'),use_container_width=True)
        st.dataframe(grafica_calidad.sort_values(['Periodo','Entidad']),use_container_width=True,hide_index=True,column_config={'Cumplimiento %':st.column_config.NumberColumn(format='%.1f%%')})

    st.markdown('### Cumplimiento SPAC Producción')
    if grafica_produccion.empty or grafica_produccion['Cumplimiento %'].dropna().empty:
        st.info('Ingresa y guarda Datos cargados de Producción para visualizar la gráfica.')
    else:
        st.line_chart(grafica_produccion.pivot_table(index='Periodo',columns='Entidad',values='Cumplimiento %',aggfunc='first'),use_container_width=True)
        st.dataframe(grafica_produccion.sort_values(['Periodo','Entidad']),use_container_width=True,hide_index=True,column_config={'Cumplimiento %':st.column_config.NumberColumn(format='%.1f%%')})
