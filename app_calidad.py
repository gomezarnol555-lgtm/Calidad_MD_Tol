...
from base_datos import inicializar_base, diagnostico_conexion
...
def main():
    ...
    try:
        ...
        st.info(mensaje)
        st.markdown("**Revisa:** Settings → Secrets, ejecuta `supabase_esquema.sql` y reinicia la aplicación.")
        st.stop()
    if FORCE_RESET_ADMIN and ADMIN_PASS:reset_admin()
    user=login();cambiar_password_obligatoria();styles(False)
    a,b=st.columns([.19,.81],gap='large')
    with a:left_menu()
    with b:
