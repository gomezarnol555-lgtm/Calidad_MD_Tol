...
def reset_admin():
    ...
    c = conn()
    cur = c.cursor()
    cur.execute(sentencia_datos('SEGURIDAD_0001'), (ADMIN_USER,))
    exists = cur.fetchone()
    if exists:
        cur.execute(sentencia_datos('SEGURIDAD_0002'), ('Administrador del sistema', pw, 'desarrollador', ADMIN_USER))
    else:
        cur.execute(sentencia_datos('SEGURIDAD_0003'), (ADMIN_USER, 'Administrador del sistema', pw, 'desarrollador', now_iso()))
    c.commit()
    c.close()
...
def auth_user(u, p):
    ...
    c = conn()
    cur = c.cursor()
    cur.execute(sentencia_datos('SEGURIDAD_0004'), (usuario,))
    row = cur.fetchone()
    if not row:
        c.close()
    ...
    if check_password(p, pw):
        cur.execute(sentencia_datos('SEGURIDAD_0005'), (uid,))
        c.commit()
        c.close()
        return {'estado': 'ok', 'usuario': user, 'nombre': nombre, 'rol': rol, 'requiere_cambio_pass': int(requiere or 0)}
    nuevos = intentos + 1
    bloqueado = nuevos >= 8
    cur.execute(sentencia_datos('SEGURIDAD_0006'), (min(nuevos, 8), 0 if bloqueado else 1, uid))
    c.commit()
    c.close()
    if bloqueado:
