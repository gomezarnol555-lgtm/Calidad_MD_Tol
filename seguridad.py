...
def hash_password(p):
    salt = os.urandom(16)
    key = hashlib.pbkdf2_hmac('sha256', p.encode(), salt, 120000)
    return base64.b64encode(salt + key).decode()

def check_password(p, stored):
    try:
        raw = base64.b64decode(str(stored).encode())
        return hashlib.pbkdf2_hmac('sha256', p.encode(), raw[:16], 120000) == raw[16:]
    except Exception:
        return False
...
def reset_admin():
    ...
    c = transaccion()
    cur = c.cursor()
    cur.execute(operacion_datos('SEGURIDAD_0001'), (ADMIN_USER,))
    exists = cur.fetchone()
    if exists:
        cur.execute(operacion_datos('SEGURIDAD_0002'), ('Administrador del sistema', pw, 'desarrollador', ADMIN_USER))
    else:
        cur.execute(operacion_datos('SEGURIDAD_0003'), (ADMIN_USER, 'Administrador del sistema', pw, 'desarrollador', now_iso()))
    c.commit()
    c.close()
...
def auth_user(u, p):
    ...
    c = transaccion()
    cur = c.cursor()
    cur.execute(operacion_datos('SEGURIDAD_0004'), (usuario,))
    row = cur.fetchone()
    if not row:
        c.close()
    ...
    if check_password(p, pw):
        cur.execute(operacion_datos('SEGURIDAD_0005'), (uid,))
        c.commit()
        c.close()
        return {'estado': 'ok', 'usuario': user, 'nombre': nombre, 'rol': rol, 'requiere_cambio_pass': int(requiere or 0)}
    nuevos = intentos + 1
    bloqueado = nuevos >= 8
    cur.execute(operacion_datos('SEGURIDAD_0006'), (min(nuevos, 8), 0 if bloqueado else 1, uid))
    c.commit()
    c.close()
    if bloqueado:
