...
def reset_admin():
    ...
    exists = cur.fetchone()
    if exists:
        cur.execute('UPDATE usuarios SET nombre=?, password_hash=?, rol=?, activo=1, intentos_fallidos=0, requiere_cambio_pass=0 WHERE usuario=?', ('Administrador del sistema', pw, 'desarrollador', ADMIN_USER))
    else:
        cur.execute('INSERT INTO usuarios(usuario,nombre,password_hash,rol,activo,intentos_fallidos,requiere_cambio_pass,creado_en) VALUES(?,?,?,?,1,0,0,?)', (ADMIN_USER, 'Administrador del sistema', pw, 'desarrollador', now_iso()))
    c.commit()
    c.close()
