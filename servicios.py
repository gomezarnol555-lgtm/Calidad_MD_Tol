...
def catalog(categoria):
    """Obtengo un catálogo activo mediante el contrato neutral de datos."""
    df = consultar(
        tabla='catalogos',
        columnas=['valor', 'orden'],
        filtros={'categoria': str(categoria).strip(), 'activo': 1},
        ordenar_por=[('orden', 'asc'), ('valor', 'asc')],
    )
    df = _dataframe_con_columnas(df, ['valor', 'orden'])
    valores = (
        df['valor'].dropna().astype(str).str.strip()
        .loc[lambda serie: serie.ne('')].tolist()
    )
    if categoria == 'nave':
        valores = [normalizar_nave(valor) for valor in valores]
