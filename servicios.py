...
def nave_por_catalogo(linea, grupo=''):
    d = consultar(operacion_datos('SERVICIOS_0007'))
...
def formato_entrega(nave, tipo='PROCESO'):
    df = consultar(operacion_datos('SERVICIOS_0008'), (nave, tipo))
    if tipo == 'ANALISIS':
...
def formato_seguimientos():
    df = consultar(operacion_datos('SERVICIOS_0009'))
    return df['nombre'].dropna().astype(str).str.strip().loc[lambda x: x.ne('')].tolist() if not df.empty else []

def configuracion_seguimientos():
    bloques = consultar(operacion_datos('SERVICIOS_0010'))
    resultado = []
    for bloque in bloques.itertuples():
        campos = consultar(operacion_datos('SERVICIOS_0011'), (int(bloque.id),))
        definiciones = []
...
def catalog(cat):
    df = consultar(operacion_datos('SERVICIOS_0012'), (cat,))
...
def guardar_matriz(eid, fecha, analista, carga, tot):
    ...
    crear(operacion_datos('SERVICIOS_0013'), (fecha, analista_limpio, eid, carga, tot.get('Nave 1', 0), tot.get('Nave 2', 0), tot.get('Nave 3', 0), now_iso()))

def audit(u, a, d):
    crear(operacion_datos('SERVICIOS_0014'), (u, a, d, now_iso()))

def new_folio():
    pref = f'PNC-{datetime.now().year}-'
    df = consultar(operacion_datos('SERVICIOS_0015'), (f'{pref}%',))
    n = 0
    if not df.empty:
        try:
...
def aplicar_ajustes_diarios(tabla, tipo_entidad, columna_entidad):
    ajustes = consultar(operacion_datos('SERVICIOS_0016'), (tipo_entidad,))
...
def datos_grafica_cumplimiento(tipo_entidad, periodo_tipo):
    cargados = consultar(operacion_datos('SERVICIOS_0017'), (tipo_entidad, periodo_tipo))
    if cargados.empty:
        return pd.DataFrame(columns=['Periodo', 'Entidad', 'Cumplimiento %'])
    registros = consultar(operacion_datos('SERVICIOS_0018'))
    registros['fecha_dt'] = pd.to_datetime(registros['fecha'], errors='coerce').dt.date
    ajustes = consultar(operacion_datos('SERVICIOS_0019'), (tipo_entidad,))
    resultado = []
    for fila in cargados.itertuples():
        ...
        if tipo_entidad == 'ANALISTA':
            ...
            calidad = consultar(operacion_datos('SERVICIOS_0020'))
