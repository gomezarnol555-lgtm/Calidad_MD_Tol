...
def configuracion_seguimientos():
    """Obtiene la configuracion activa y garantiza un bloque visible de seguimiento."""
    columnas_bloque = ['id', 'nombre']
    columnas_campo = ['id', 'nombre', 'tipo_campo', 'opciones', 'obligatorio', 'orden']
    ...
    bloques = _dataframe_con_columnas(bloques, columnas_bloque)

    resultado = []
    for bloque in bloques.itertuples():
        nombre_bloque = str(getattr(bloque, 'nombre', '') or '').strip()
        ...
        campos = _dataframe_con_columnas(campos, columnas_campo)
        definiciones = []
        for campo in campos.itertuples():
            nombre_campo = str(getattr(campo, 'nombre', '') or '').strip()
            if not nombre_campo:
                continue
            tipo_campo = str(getattr(campo, 'tipo_campo', '') or 'Texto').strip()
            opciones = [
                opcion.strip()
                for opcion in str(getattr(campo, 'opciones', '') or '').split('|')
                if opcion.strip()
            ]
            definiciones.append({
                'id': int(campo.id),
                ...
                'tipo': tipo_campo,
                'opciones': opciones,
                'obligatorio': bool(getattr(campo, 'obligatorio', False)),
            })
        # Un seguimiento configurado sin campos no debe desaparecer del formato.
        if not definiciones:
            definiciones = _campos_seguimiento_predeterminados()
        resultado.append({
            'id': int(bloque.id),
    ...
    # Proteccion funcional para instalaciones donde el catalogo aun esta vacio.
    if not resultado:
        resultado = [{
            'id': 0,
            'nombre': 'Seguimientos generales',
            'campos': _campos_seguimiento_predeterminados(),
        }]
    return resultado


def _campos_seguimiento_predeterminados():
    """Campos compatibles con el guardado historico de entregas de turno."""
    return [
