...
def catalog(categoria):
    """Obtengo una lista activa desde el contrato neutral de acceso a datos."""
    categoria_limpia = str(categoria or '').strip()
    if not categoria_limpia:
        return []
    dataframe = consultar(
        tabla='catalogos',
        columnas=['valor', 'orden'],
        filtros={'categoria': categoria_limpia, 'activo': 1},
        ordenar_por=[('orden', 'asc'), ('valor', 'asc')],
    )
    if dataframe.empty or 'valor' not in dataframe.columns:
        return []
    valores = dataframe['valor'].dropna().astype(str).str.strip()
    valores = valores[valores.ne('')].tolist()
    if categoria_limpia == 'nave':
        valores = [normalizar_nave(valor) for valor in valores if normalizar_nave(valor)]
